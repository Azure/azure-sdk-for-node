 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _productsByProject(billingAccountId, billingProfileId, startDate, endDate, options, callback) {
   /* jshint validthis: true */
  let client = this;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class ConsumptionManagementClient extends ServiceClient {
   ...
   * @reject {Error} - The error object.
   */
  productsByProjectWithHttpOperationResponse(billingAccountId, billingProfileId, startDate, endDate, options) {
    let client = this;
    let self = this;
    return new Promise((resolve, reject) => {
      self._productsByProject(billingAccountId, billingProfileId, startDate, endDate, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  productsByProject(billingAccountId, billingProfileId, startDate, endDate, options, optionalCallback) {
    let client = this;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._productsByProject(billingAccountId, billingProfileId, startDate, endDate, options, optionalCallback);
    }
  }
