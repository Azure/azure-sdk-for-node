 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _productsByProject(billingAccountId, projectId, startDate, endDate, options, callback) {
   /* jshint validthis: true */
  let client = this;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/billingAccounts/{billingAccountId}/projects/{projectId}/providers/Microsoft.Consumption/products';
  requestUrl = requestUrl.replace('{billingAccountId}', encodeURIComponent(billingAccountId));
...
class ConsumptionManagementClient extends ServiceClient {
  ...
  constructor(credentials, baseUri, options) {
    ...
    this._productsByProject = _productsByProject;
   ...
   * @reject {Error} - The error object.
   */
  productsByProjectWithHttpOperationResponse(billingAccountId, projectId, startDate, endDate, options) {
    let client = this;
    let self = this;
    return new Promise((resolve, reject) => {
      self._productsByProject(billingAccountId, projectId, startDate, endDate, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  productsByProject(billingAccountId, projectId, startDate, endDate, options, optionalCallback) {
    let client = this;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._productsByProject(billingAccountId, projectId, startDate, endDate, options, optionalCallback);
    }
  }
