 ...
 */
function _list(scope, grain, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + '{scope}/providers/Microsoft.Consumption/reservationSummaries';
...
class ReservationsSummaries {
   ...
   */
  listWithHttpOperationResponse(scope, grain, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._list(scope, grain, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  list(scope, grain, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._list(scope, grain, options, optionalCallback);
    }
  }
