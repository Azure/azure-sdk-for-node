...
function _list(billingAccountId, startDate, endDate, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/billingAccounts/{billingAccountId}/providers/Microsoft.Consumption/credits';
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _download(billingAccountId, startDate, endDate, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/billingAccounts/{billingAccountId}/providers/Microsoft.Consumption/credits/download';
  requestUrl = requestUrl.replace('{billingAccountId}', encodeURIComponent(billingAccountId));
  let queryParameters = [];
  queryParameters.push('startDate=' + encodeURIComponent(startDate));
  queryParameters.push('endDate=' + encodeURIComponent(endDate));
...
/** Class representing a CreditsByBillingAccount. */
class CreditsByBillingAccount {
  /**
   * Create a CreditsByBillingAccount.
   ...
   * @reject {Error} - The error object.
   */
  downloadWithHttpOperationResponse(billingAccountId, startDate, endDate, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._download(billingAccountId, startDate, endDate, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  download(billingAccountId, startDate, endDate, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._download(billingAccountId, startDate, endDate, options, optionalCallback);
    }
  }
...
module.exports = CreditsByBillingAccount;
