 ...
 */
function _list(scope, filter, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + '{scope}/providers/Microsoft.Consumption/reservationDetails';
...
class ReservationsDetails {
   ...
   */
  listWithHttpOperationResponse(scope, filter, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._list(scope, filter, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  list(scope, filter, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._list(scope, filter, options, optionalCallback);
    }
  }
