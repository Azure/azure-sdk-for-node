 ...
 */
function _listByScope(scope, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + '{scope}/providers/Microsoft.Consumption/charges';
...
class Charges {
  ...
  constructor(client) {
    this.client = client;
    this._listByScope = _listByScope;
  }
   ...
   */
  listByScopeWithHttpOperationResponse(scope, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._listByScope(scope, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  listByScope(scope, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._listByScope(scope, options, optionalCallback);
    }
  }
