 ...
 */
function _get(scope, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + '{scope}/providers/Microsoft.Consumption/tags';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['TagsResult']().mapper();
...
class Tags {
   ...
   */
  getWithHttpOperationResponse(scope, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(scope, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(scope, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._get(scope, options, optionalCallback);
    }
  }
