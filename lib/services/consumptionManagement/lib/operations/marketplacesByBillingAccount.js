...
function _list(billingAccountId, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.CostManagement/billingAccounts/{billingAccountId}/providers/Microsoft.Consumption/marketplaces';
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _listByBillingPeriod(billingAccountId, billingPeriodName, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.CostManagement/billingAccounts/{billingAccountId}/providers/Microsoft.Billing/billingPeriods/{billingPeriodName}/providers/Microsoft.Consumption/marketplaces';
  requestUrl = requestUrl.replace('{billingAccountId}', encodeURIComponent(billingAccountId));
...
/** Class representing a MarketplacesByBillingAccount. */
class MarketplacesByBillingAccount {
  /**
   * Create a MarketplacesByBillingAccount.
   ...
   * @reject {Error} - The error object.
   */
  listByBillingPeriodWithHttpOperationResponse(billingAccountId, billingPeriodName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._listByBillingPeriod(billingAccountId, billingPeriodName, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  listByBillingPeriod(billingAccountId, billingPeriodName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._listByBillingPeriod(billingAccountId, billingPeriodName, options, optionalCallback);
    }
  }
...
module.exports = MarketplacesByBillingAccount;
