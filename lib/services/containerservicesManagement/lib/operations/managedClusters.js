 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _listCredential(resourceGroupName, resourceName, roleName, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class ManagedClusters {
  ...
  constructor(client) {
    ...
    this._listCredential = _listCredential;
   ...
   * @reject {Error} - The error object.
   */
  listCredentialWithHttpOperationResponse(resourceGroupName, resourceName, roleName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._listCredential(resourceGroupName, resourceName, roleName, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  listCredential(resourceGroupName, resourceName, roleName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._listCredential(resourceGroupName, resourceName, roleName, options, optionalCallback);
    }
  }
