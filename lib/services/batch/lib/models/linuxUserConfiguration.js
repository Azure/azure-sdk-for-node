...
class LinuxUserConfiguration {
   ...
   * @property {number} [gid] The group ID for the user Account. The uid and
   * gid properties must be specified together or not at all. If not specified
   * the underlying operating system picks the gid.
   * @property {string} [sshPrivateKey] The SSH private key for the user
   * Account. The private key must not be password protected. The private key
   * is used to automatically configure asymmetric-key based authentication for
   * SSH between Compute Nodes in a Linux Pool when the Pool's
