...
class ComputeNode {
   ...
   * for task scheduling. enabled - Tasks can be scheduled on the node.
   ...
   * adding a task to request that the task be scheduled close to this compute
   * node.
   ...
   * on the compute node. This includes Job Preparation, Job Release and Job
   ...
   * @member {string} [startTaskInfo.state] running - The start task is
   * currently running. completed - The start task has exited with exit code 0,
   * or the start task has failed and the retry limit has reached, or the start
   * task process did not run due to scheduling errors. Possible values
   ...
   * @member {number} [startTaskInfo.retryCount] The task is retried if it
