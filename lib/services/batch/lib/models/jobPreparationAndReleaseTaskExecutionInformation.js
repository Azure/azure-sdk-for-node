...
class JobPreparationAndReleaseTaskExecutionInformation {
   ...
   * @member {string} [jobPreparationTaskExecutionInfo.state] running - the
   * task is currently running (including retrying). completed - the task has
   * exited with exit code 0, or the task has exhausted its retry limit, or the
   ...
   * @member {string}
   * [jobPreparationTaskExecutionInfo.schedulingError.category] Possible values
   ...
   * currently running (including retrying). completed - the task has exited,
   * or the Batch service was unable to start the task due to scheduling
