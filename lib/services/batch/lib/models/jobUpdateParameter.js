...
class JobUpdateParameter {
   ...
   * the pool lifetime scope is jobSchedule level, the Batch service keeps
   * track of the last autopool created for the job schedule, and deletes that
   * pool when the job schedule completes. Batch will also delete this pool if
   * the user updates the auto pool specification in a way that changes this
   ...
   * its poolAllocationMode property set to 'UserSubscription'. This property
   * and imageReference are mutually exclusive and one of the properties must
   * be specified.
   * @member {array}
   ...
   * within the same subscription and same region as the Batch account. For
   * best performance, it is recommended that each VHD resides in a separate
   * Azure Storage account. Each VHD can serve upto 20 Windows compute nodes or
   * 40 Linux compute nodes. You must supply enough VHD URIs to satisfy the
   ...
   * @member {object}
   * [poolInfo.autoPoolSpecification.pool.taskSchedulingPolicy] How tasks are
   ...
   * unusable. This property can only be specified for pools created with a
   ...
   * is created on every node in a pool. task - specifies that the service
   ...
   * it. This condition can be detected via the node state and scheduling error
