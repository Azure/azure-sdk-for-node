 ...
 */
function _getCapability(location, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'subscriptions/{subscriptionId}/providers/Microsoft.DataLakeStore/locations/{location}/capability';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['CapabilityInformation']().mapper();
...
class Locations {
  ...
  constructor(client) {
    this.client = client;
    this._getCapability = _getCapability;
  }
   ...
   */
  getCapabilityWithHttpOperationResponse(location, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getCapability(location, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getCapability(location, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._getCapability(location, options, optionalCallback);
    }
  }
