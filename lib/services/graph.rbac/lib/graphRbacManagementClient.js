...
var GroupOperations = ( /** @lends GroupOperations */ function() {
   ...
   * 
   * @param {string} [displayName] Display name to filter results.
   * 
   * @param {function} callback
   * 
   * @returns {Stream} The response stream.
   */
  GroupOperations.prototype.list = function(mail, displayName, callback) {
    if (callback === null || callback === undefined) {
      throw new Error('callback cannot be null.');
    }
    // Validate
    ...
    if (displayName) {
      url2 = url2 + '&$filter=displayName eq \'' + encodeURIComponent(displayName ? displayName.trim() : '') + '\'';
    }
    url2 = url2 + '&api-version=1.42-previewInternal';
    var baseUrl = this.client.baseUri;
    // Trim '/' character from the end of baseUrl and beginning of url.
...
var UserOperations = ( /** @lends UserOperations */ function() {
   ...
   * @returns {Stream} The response stream.
   */
  UserOperations.prototype.list = function(mail, displayName, callback) {
    if (callback === null || callback === undefined) {
      throw new Error('callback cannot be null.');
    }
    // Validate
