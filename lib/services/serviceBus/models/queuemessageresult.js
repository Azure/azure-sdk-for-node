...
QueueMessageResult.parse = function (responseObject) {
  ...
  queueMessageResult.messagetext = responseObject.body;
  ...
  for (var header in responseObject.headers) {
    ...
    if (header !== HeaderConstants.CONTENT_TYPE &&
      ...
      customProperties[header] = QueueMessageResult.propertyFromString(responseObject.headers[header]);
    }
  }
...
QueueMessageResult.propertyFromString = function (value) {
  if (value === null) {
    return null;
  }
...
QueueMessageResult.isRFC1123 = function (value) {
  ...
  return azureutil.isValidDate(date);
};