...
function WnsService(notificationHubService) {
  ...
  var addCreateAndUpdateRegistrationMethod = function (createRegistrationFunctionName, templateName) {
    function createRegistrationXml(rawArgs) {
      ...
      return self._createTemplateBody(channel, payload, options);
    }
...
WnsService.prototype.createRawTemplateRegistration = function (channel, template, optionsOrCallback, callback) {
  ...
  var registrationXml = this._createTemplateBody(channel, template, options);
...
* @return {undefined}
*/
WnsService.prototype._createTemplateBody = function (channel, template, options) {
  var registration = {
    ChannelUri: channel,
  ...
  if (options) {
    ...
    if (options.tags) {
      ...
      registration['WnsHeaders'].push({ WnsHeader: { Header: 'X-WNS-Tag', Value: options.tags } });
    }
  }
