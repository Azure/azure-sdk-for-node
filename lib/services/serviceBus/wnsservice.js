...
function WnsService(notificationHubService) {
  ...
  var addCreateAndUpdateRegistrationMethod = function (createRegistrationFunctionName, templateName) {
    function createRegistrationXml(rawArgs) {
      ...
      // TODO: check if this should be WindowsTemplateRegistrationDescription
      return registrationResult.serialize('WindowsRegistrationDescription', registration);
    }
    ...
    self[createRegistrationName] = function () {
      ...
      var webResource = WebResource.post(self.notificationHubService.hubName + '/registrations')
        .addOptionalHeader(HeaderConstants.CONTENT_TYPE, 'application/atom+xml;type=entry;charset=utf-8')
        .addOptionalHeader(HeaderConstants.CONTENT_LENGTH, Buffer.byteLength(registrationXml, 'utf8'))
...
* @return {undefined}
*/
WnsService.prototype.updatesRawTemplateRegistration = function (channel, template, optionsOrCallback, callback) {
  var options;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { options = o; callback = c; });
  ...
  var webResource = WebResource.put(this.notificationHubService.hubName + '/registrations')
    .addOptionalHeader(HeaderConstants.CONTENT_TYPE, 'application/atom+xml;type=entry;charset=utf-8')
    .addOptionalHeader(HeaderConstants.CONTENT_LENGTH, Buffer.byteLength(registrationXml, 'utf8'))
    .withOkCode(HttpConstants.HttpResponseCodes.Ok);
...
/**
* Creates a raw template registration body.
*
* @param {string}             channel                      The device channel uri.
* @param {json}               template                     The template for the registration.
...
* @return {undefined}
*/
WnsService.prototype._createRawTemplateBody = function (channel, template, options) {
  var registration = registrationResult.serialize('WindowsRegistrationDescription', {
    ChannelUri: channel
  });
