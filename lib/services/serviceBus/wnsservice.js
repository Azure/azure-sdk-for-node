...
function WnsService(notificationHubService) {
  ...
  var addSendMethod = function (createFunctionName, templateName) {
    ...
    self[sendName] = function () {
      ...
      self.send(hub, payload, 'wns/' + type, options, callback);
    };
  };
...
* @return {undefined}
*/
WnsService.prototype.sendBadge = function (hub, value, optionsOrCallback, callback) {
  var realValue;
  var realVersion;
  ...
  this.send(hub, payload, 'wns/badge', optionsOrCallback, callback);
};
...
* @return {undefined}
*/
WnsService.prototype.send = function (hub, payload, type, optionsOrCallback, callback) {
  var options;
  ...
  options[HeaderConstants.SERVICE_BUS_NOTIFICATION_FORMAT] = 'windows';
  ...
  options[HeaderConstants.SERVICE_BUS_X_WNS_TYPE] = type;
  if (type === 'wns/raw' && !options[HeaderConstants.CONTENT_TYPE]) {
    options[HeaderConstants.CONTENT_TYPE] = 'application/octet-stream';
  }
