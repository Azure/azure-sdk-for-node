...
ApnsService.prototype.createTemplateRegistration = function (token, template, optionsOrCallback, callback) {
  ...
  var registrationXml = this._createTemplateBody(token, template);
...
* @return {undefined}
*/
ApnsService.prototype.updateTemplateRegistration = function (registrationId, token, template, optionsOrCallback, callback) {
  var options;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { options = o; callback = c; });
...
*/
ApnsService.prototype._createTemplateBody = function (token, template) {
  var registration = {
    DeviceToken: token
  };

  if (template.expiry) {
    registration['Expiry'] = template.expiry;
    delete template.expiry;
  }

  if (!_.isString(template)) {
    template = JSON.stringify({ aps: template });
  }
