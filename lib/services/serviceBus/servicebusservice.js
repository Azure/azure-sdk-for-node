...
ServiceBusService.prototype.createQueueIfNotExists = function (queuePath, optionsOrCallback, callback) {
  ...
  this.createQueue(queuePath, options, function (err, response) {
    // Check if topic was actually created.
    var created = (response.statusCode === HttpConstants.HttpResponseCodes.CREATED_CODE);

    if (response.statusCode === HttpConstants.HttpResponseCodes.CREATED_CODE || response.statusCode === HttpConstants.HttpResponseCodes.CONFLICT_CODE) {
      // If it was created before, there was no actual error.
      err = null;
    }

    callback(err, created, response);
  });
};
...
ServiceBusService.prototype.createTopicIfNotExists = function (topic, optionsOrCallback, callback) {
  ...
  this.createTopic(topic, options, function (err, response) {
    // Check if topic was actually created.
    var created = (response.statusCode === HttpConstants.HttpResponseCodes.CREATED_CODE);
