...
function ServiceBusServiceBase(configOrNamespaceOrConnectionStringOrSettings, accessKey, issuer, acsNamespace, host, authenticationProvider) {
  ...
  if(!configOrNamespaceOrConnectionStringOrSettings || _.isFunction(configOrNamespaceOrConnectionStringOrSettings)) {
    ...
    if (!configOrNamespaceOrConnectionStringOrSettings) {
      configOrNamespaceOrConnectionStringOrSettings = process.env[ServiceClient.EnvironmentVariables.AZURE_SERVICEBUS_NAMESPACE];
    }
    ...
    if (!acsNamespace) {
      ...
      if (!acsNamespace) {
        acsNamespace = configOrNamespaceOrConnectionStringOrSettings + ServiceClient.DEFAULT_WRAP_NAMESPACE_SUFFIX;
      }
    }

    var endpoint = url.format({ protocol: 'https:', port: 443, hostname: configOrNamespaceOrConnectionStringOrSettings + '.' + ServiceClient.CLOUD_SERVICEBUS_HOST });
