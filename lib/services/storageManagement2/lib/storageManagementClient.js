...
var StorageAccountOperations = ( /** @lends StorageAccountOperations */ function() {
   ...
   * 
   * @param {AccountType} parameters.accountType Gets or sets one of the
   * following account types: StandardLRS, StandardGRS, StandardRAGRS or
   * StandardZRS. Note that StandardZRS accounts cannot be changed to other
   * account types, and other account types cannot be changed to StandardZRS
   * using UpdateAccountType.
   * 
   ...
   * characters and value no greater than 256 characters. The resource
   * provider is expected to store these tags with the resource.
   * 
   * @param {function} callback
   * 
   * @returns {Stream} The response stream.
  ...
  /**
   * Lists all the storage accounts available under the given resource stack.
   ...
   * @returns {Stream} The response stream.
   */
  StorageAccountOperations.prototype.list = function(resourceGroupName, callback) {
    if (callback === null || callback === undefined) {
      throw new Error('callback cannot be null.');
    }
    // Validate
   ...
   * account. Only one property can be changed at a time using this API. Use
   * the RegenerateKeys operation to change the keys for a storage account.
   ...
   * 
   * @param {AccountType} [parameters.accountType] Gets or sets one of the
   * following account types: StandardLRS, StandardGRS, StandardRAGRS. Note
   * that StandardZRS accounts cannot be changed to other account types, and
   * other account types cannot be changed to StandardZRS.
   * 
   * @param {CustomDomain} [parameters.customDomain] Gets or sets the custom
   * domain assigned to the storage account.
   * 
   * @param {string} [parameters.customDomain.name] Gets or sets the custom
   * domain name. Name is the fully qualified domain name (FQDN) of the custom
   * domain name you want to assign to this storage account
   * 
   * @param {boolean} [parameters.customDomain.useSubDomain] Gets or sets
   * whether or not to use a sub domain. This should only be set on update
   * reqests and for update responses.
   * 
   * @param {object} [parameters.tags] Gets or sets a list of key value pairs
   * that describe the resource. These tags can be used in viewing and
   * grouping this resource (across resource groups). A maximum of 15 tags can
   * be provided for a resource. Each tag must have a key no greater than 128
   * characters and value no greater than 256 characters. The resource
   * provider is expected to store these tags with the resource. This is a
