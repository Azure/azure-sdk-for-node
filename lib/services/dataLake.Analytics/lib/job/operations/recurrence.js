...
function _list(accountName, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'recurrences';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['JobRecurrenceInformationListResult']().mapper();
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _get(accountName, recurrenceIdentity, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    ...
    if (recurrenceIdentity === null || recurrenceIdentity === undefined || typeof recurrenceIdentity.valueOf() !== 'string' || !msRest.isValidUuid(recurrenceIdentity)) {
      throw new Error('recurrenceIdentity cannot be null or undefined and it must be of type string and must be a valid uuid.');
    }
    if (startDateTime && !(startDateTime instanceof Date ||
        (typeof startDateTime.valueOf() === 'string' && !isNaN(Date.parse(startDateTime))))) {
          throw new Error('startDateTime must be of type date.');
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'recurrences/{recurrenceIdentity}';
  requestUrl = requestUrl.replace('{accountName}', accountName);
  requestUrl = requestUrl.replace('{adlaJobDnsSuffix}', this.client.adlaJobDnsSuffix);
  requestUrl = requestUrl.replace('{recurrenceIdentity}', encodeURIComponent(recurrenceIdentity.toString()));
  let queryParameters = [];
  if (startDateTime !== null && startDateTime !== undefined) {
    queryParameters.push('startDateTime=' + encodeURIComponent(client.serializeObject(startDateTime)));
  }
  ...
  return client.pipeline(httpRequest, (err, response, responseBody) => {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['JobRecurrenceInformation']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        let deserializationError = new Error(`Error ${error} occurred in deserializing the responseBody - ${responseBody}`);
...
/** Class representing a Recurrence. */
class Recurrence {
  /**
   * Create a Recurrence.
   ...
   * @reject {Error} - The error object.
   */
  getWithHttpOperationResponse(accountName, recurrenceIdentity, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(accountName, recurrenceIdentity, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(accountName, recurrenceIdentity, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._get(accountName, recurrenceIdentity, options, optionalCallback);
    }
  }
