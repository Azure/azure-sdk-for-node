...
function _list(accountName, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'pipelines';
  ...
  if (startDateTime !== null && startDateTime !== undefined) {
    queryParameters.push('startDateTime=' + encodeURIComponent(client.serializeObject(startDateTime)));
  }
  if (endDateTime !== null && endDateTime !== undefined) {
    queryParameters.push('endDateTime=' + encodeURIComponent(client.serializeObject(endDateTime)));
  }
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['JobPipelineInformationListResult']().mapper();
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _get(accountName, pipelineIdentity, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'pipelines/{pipelineIdentity}';
  requestUrl = requestUrl.replace('{accountName}', accountName);
  requestUrl = requestUrl.replace('{adlaJobDnsSuffix}', this.client.adlaJobDnsSuffix);
  requestUrl = requestUrl.replace('{pipelineIdentity}', encodeURIComponent(pipelineIdentity.toString()));
  let queryParameters = [];
  if (startDateTime !== null && startDateTime !== undefined) {
    queryParameters.push('startDateTime=' + encodeURIComponent(client.serializeObject(startDateTime)));
  }
  ...
  return client.pipeline(httpRequest, (err, response, responseBody) => {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['JobPipelineInformation']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        let deserializationError = new Error(`Error ${error} occurred in deserializing the responseBody - ${responseBody}`);
...
/** Class representing a PipelineModel. */
class PipelineModel {
  /**
   ...
   * @reject {Error} - The error object.
   */
  getWithHttpOperationResponse(accountName, pipelineIdentity, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(accountName, pipelineIdentity, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(accountName, pipelineIdentity, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._get(accountName, pipelineIdentity, options, optionalCallback);
    }
  }
