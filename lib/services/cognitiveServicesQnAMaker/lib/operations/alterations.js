...
function _get(options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'alterations';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['WordAlterationsDTO']().mapper();
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _replace(wordAlterations, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    ...
    if (wordAlterations === null || wordAlterations === undefined) {
      throw new Error('wordAlterations cannot be null or undefined.');
    }
  } catch (error) {
    return callback(error);
  }
  ...
  try {
    if (wordAlterations !== null && wordAlterations !== undefined) {
      let requestModelMapper = new client.models['WordAlterationsDTO']().mapper();
      requestModel = client.serialize(requestModelMapper, wordAlterations, 'wordAlterations');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(wordAlterations, null, 2)}.`);
...
class Alterations {
   ...
   * @reject {Error} - The error object.
   */
  replaceWithHttpOperationResponse(wordAlterations, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._replace(wordAlterations, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  replace(wordAlterations, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._replace(wordAlterations, options, optionalCallback);
    }
  }
