 ...
 */
function _get(resourceGroupName, serverName, restorableDroppededDatabaseId, options, callback) {
  ...
  try {
    ...
    if (restorableDroppededDatabaseId === null || restorableDroppededDatabaseId === undefined || typeof restorableDroppededDatabaseId.valueOf() !== 'string') {
      throw new Error('restorableDroppededDatabaseId cannot be null or undefined and it must be of type string.');
    }
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Sql/servers/{serverName}/restorableDroppedDatabases/{restorableDroppededDatabaseId}';
  requestUrl = requestUrl.replace('{subscriptionId}', encodeURIComponent(this.client.subscriptionId));
  requestUrl = requestUrl.replace('{resourceGroupName}', encodeURIComponent(resourceGroupName));
  requestUrl = requestUrl.replace('{serverName}', encodeURIComponent(serverName));
  requestUrl = requestUrl.replace('{restorableDroppededDatabaseId}', encodeURIComponent(restorableDroppededDatabaseId));
  let queryParameters = [];
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['RestorableDroppedDatabase']().mapper();
...
function _listByServer(resourceGroupName, serverName, options, callback) {
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Sql/servers/{serverName}/restorableDroppedDatabases';
  requestUrl = requestUrl.replace('{subscriptionId}', encodeURIComponent(this.client.subscriptionId));
  requestUrl = requestUrl.replace('{resourceGroupName}', encodeURIComponent(resourceGroupName));
  requestUrl = requestUrl.replace('{serverName}', encodeURIComponent(serverName));
  let queryParameters = [];
  ...
  return client.pipeline(httpRequest, (err, response, responseBody) => {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['RestorableDroppedDatabaseListResult']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        let deserializationError = new Error(`Error ${error} occurred in deserializing the responseBody - ${responseBody}`);
...
class RestorableDroppedDatabases {
   ...
   */
  getWithHttpOperationResponse(resourceGroupName, serverName, restorableDroppededDatabaseId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(resourceGroupName, serverName, restorableDroppededDatabaseId, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(resourceGroupName, serverName, restorableDroppededDatabaseId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._get(resourceGroupName, serverName, restorableDroppededDatabaseId, options, optionalCallback);
    }
  }
...
module.exports = RestorableDroppedDatabases;
