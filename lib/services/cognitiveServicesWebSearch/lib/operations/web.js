...
function _search(query, options, callback) {
  ...
  if (promote !== null && promote !== undefined) {
    queryParameters.push('promote=' + promote.map(x => encodeURIComponent(x)).join(','));
  }
  queryParameters.push('q=' + encodeURIComponent(query));
  if (responseFilter !== null && responseFilter !== undefined) {
    queryParameters.push('responseFilter=' + responseFilter.map(x => encodeURIComponent(x)).join(','));
  }
...
class Web {
  /**
   * Create a Web.
   * @param {WebSearchClient} client Reference to the service client.
   */
  constructor(client) {
    this.client = client;
