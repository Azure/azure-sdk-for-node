    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['OAuth2']().mapper();
...
function _post(options, callback) {
  ...
  try {
    if (body !== null && body !== undefined) {
      let requestModelMapper = new client.models['OAuth2']().mapper();
...
/** Class representing a OAuth2Operations. */
class OAuth2Operations {
  /**
   * Create a OAuth2Operations.
   ...
   * @reject {Error} - The error object.
   */
  postWithHttpOperationResponse(options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._post(options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
  ...
  post(options, optionalCallback) {
    ...
    if (!optionalCallback) {
      ...
      return self._post(options, optionalCallback);
    }
  }

}

module.exports = OAuth2Operations;
