 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getGenerateRecommendationsStatus(operationId, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class Recommendations {
  ...
  constructor(client) {
    ...
    this._getGenerateRecommendationsStatus = _getGenerateRecommendationsStatus;
    this._list = _list;
    this._get = _get;
   ...
   * @reject {Error} - The error object.
   */
  getGenerateRecommendationsStatusWithHttpOperationResponse(operationId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getGenerateRecommendationsStatus(operationId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getGenerateRecommendationsStatus(operationId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._getGenerateRecommendationsStatus(operationId, options, optionalCallback);
    }
  }
