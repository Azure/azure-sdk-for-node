 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getOdataMetadata(appId, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class Events {
  ...
  constructor(client) {
    ...
    this._get = _get;
    this._getOdataMetadata = _getOdataMetadata;
  }
  ...
  getOdataMetadataWithHttpOperationResponse(appId, options) {
    ...
    let self = this;
    return new Promise((resolve, reject) => {
      self._getOdataMetadata(appId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getOdataMetadata(appId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._getOdataMetadata(appId, options, optionalCallback);
    }
  }
