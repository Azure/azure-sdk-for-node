 ...
 */
function _list(nodeName, options, callback) {
...
function _get(nodeName, applicationName, options, callback) {
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'Nodes/{nodeName}/$/GetApplications/{applicationName}';
  requestUrl = requestUrl.replace('{nodeName}', encodeURIComponent(nodeName));
  ...
  return client.pipeline(httpRequest, (err, response, responseBody) => {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['DeployedApplication']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        let deserializationError = new Error(`Error ${error} occurred in deserializing the responseBody - ${responseBody}`);
...
/** Class representing a DeployedApplications. */
class DeployedApplications {
  /**
   * Create a DeployedApplications.
   ...
   */
  listWithHttpOperationResponse(nodeName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._list(nodeName, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  list(nodeName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._list(nodeName, options, optionalCallback);
    }
  }
...
module.exports = DeployedApplications;
