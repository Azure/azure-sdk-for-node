 ...
 */
function _get(serviceName, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'Services/{serviceName}/$/GetHealth';
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _send(serviceName, serviceHealthReport, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'Services/{serviceName}/$/ReportHealth';
  requestUrl = requestUrl.replace('{serviceName}', serviceName);
  let queryParameters = [];
  if (this.client.timeout !== null && this.client.timeout !== undefined) {
    queryParameters.push('timeout=' + encodeURIComponent(this.client.timeout.toString()));
  ...
  try {
    if (serviceHealthReport !== null && serviceHealthReport !== undefined) {
      let requestModelMapper = new client.models['ServiceHealthReport']().mapper();
      requestModel = client.serialize(requestModelMapper, serviceHealthReport, 'serviceHealthReport');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(serviceHealthReport, null, 2)}.`);
 ...
 * Initializes a new instance of the ServiceHealths class.
 * @constructor
 *
...
class ServiceHealths {
   ...
   */
  getWithHttpOperationResponse(serviceName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(serviceName, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(serviceName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._get(serviceName, options, optionalCallback);
    }
  }
   ...
   * @reject {Error} - The error object.
   */
  sendWithHttpOperationResponse(serviceName, serviceHealthReport, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._send(serviceName, serviceHealthReport, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  send(serviceName, serviceHealthReport, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._send(serviceName, serviceHealthReport, options, optionalCallback);
    }
  }

}

module.exports = ServiceHealths;
