...
function _get(partitionId, options, callback) {
  ...
  let replicasHealthStateFilter = (options && options.replicasHealthStateFilter !== undefined) ? options.replicasHealthStateFilter : undefined;
  // Validate
  try {
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _send(partitionId, partitionHealthReport, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    if (partitionHealthReport !== null && partitionHealthReport !== undefined) {
      let requestModelMapper = new client.models['PartitionHealthReport']().mapper();
      requestModel = client.serialize(requestModelMapper, partitionHealthReport, 'partitionHealthReport');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(partitionHealthReport, null, 2)}.`);
...
/** Class representing a PartitionHealths. */
class PartitionHealths {
  /**
   * Create a PartitionHealths.
   ...
   * @reject {Error} - The error object.
   */
  sendWithHttpOperationResponse(partitionId, partitionHealthReport, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._send(partitionId, partitionHealthReport, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  send(partitionId, partitionHealthReport, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._send(partitionId, partitionHealthReport, options, optionalCallback);
    }
  }
