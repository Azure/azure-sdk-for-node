 ...
 */
function _get(nodeName, partitionName, replicaId, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'Nodes/{nodeName}/$/GetPartitions/{partitionName}/$/GetReplicas/{replicaId}/$/GetDetail';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['DeployedReplicaDetail']().mapper();
...
/** Class representing a DeployedReplicaDetails. */
class DeployedReplicaDetails {
  /**
   * Create a DeployedReplicaDetails.
   ...
   */
  getWithHttpOperationResponse(nodeName, partitionName, replicaId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(nodeName, partitionName, replicaId, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(nodeName, partitionName, replicaId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._get(nodeName, partitionName, replicaId, options, optionalCallback);
    }
  }
