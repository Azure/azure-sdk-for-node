 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
Nodes.prototype.disable = function (nodeName, node, options, callback) {
  var client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
    options = null;
  ...
  try {
    if (node !== null && node !== undefined) {
      var requestModelMapper = new client.models['Node']().mapper();
      requestModel = client.serialize(requestModelMapper, node, 'node');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    var serializationError = new Error(util.format('Error "%s" occurred in serializing the ' + 
        'payload - "%s"', error.message, util.inspect(node, {depth: null})));
