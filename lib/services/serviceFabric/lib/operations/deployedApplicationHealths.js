 ...
 */
function _get(nodeName, applicationName, options, callback) {
  ...
  let deployedServicePackagesHealthStateFilter = (options && options.deployedServicePackagesHealthStateFilter !== undefined) ? options.deployedServicePackagesHealthStateFilter : undefined;
  // Validate
  try {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'Nodes/{nodeName}/$/GetApplications/{applicationName}/$/GetHealth';
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _send(nodeName, applicationName, deployedApplicationHealthReport, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'Nodes/{nodeName}/$/GetApplications/{applicationName}/$/ReportHealth';
  requestUrl = requestUrl.replace('{nodeName}', encodeURIComponent(nodeName));
  requestUrl = requestUrl.replace('{applicationName}', applicationName);
  let queryParameters = [];
  if (this.client.timeout !== null && this.client.timeout !== undefined) {
  ...
  try {
    if (deployedApplicationHealthReport !== null && deployedApplicationHealthReport !== undefined) {
      let requestModelMapper = new client.models['DeployedApplicationHealthReport']().mapper();
    ...
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(deployedApplicationHealthReport, null, 2)}.`);
...
/** Class representing a DeployedApplicationHealths. */
class DeployedApplicationHealths {
  /**
   * Create a DeployedApplicationHealths.
   ...
   */
  getWithHttpOperationResponse(nodeName, applicationName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(nodeName, applicationName, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(nodeName, applicationName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._get(nodeName, applicationName, options, optionalCallback);
    }
  }
   ...
   * @reject {Error} - The error object.
   */
  sendWithHttpOperationResponse(nodeName, applicationName, deployedApplicationHealthReport, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._send(nodeName, applicationName, deployedApplicationHealthReport, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  send(nodeName, applicationName, deployedApplicationHealthReport, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._send(nodeName, applicationName, deployedApplicationHealthReport, options, optionalCallback);
    }
  }
