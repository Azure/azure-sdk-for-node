 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _send(nodeName, nodeHealthReport, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    if (nodeHealthReport !== null && nodeHealthReport !== undefined) {
      let requestModelMapper = new client.models['NodeHealthReport']().mapper();
      requestModel = client.serialize(requestModelMapper, nodeHealthReport, 'nodeHealthReport');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(nodeHealthReport, null, 2)}.`);
...
/** Class representing a NodeHealths. */
class NodeHealths {
  /**
   * Create a NodeHealths.
   ...
   * @reject {Error} - The error object.
   */
  sendWithHttpOperationResponse(nodeName, nodeHealthReport, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._send(nodeName, nodeHealthReport, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  send(nodeName, nodeHealthReport, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._send(nodeName, nodeHealthReport, options, optionalCallback);
    }
  }
