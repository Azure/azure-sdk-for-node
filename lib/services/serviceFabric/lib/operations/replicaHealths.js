 ...
 */
function _get(partitionId, replicaId, options, callback) {
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _send(partitionId, replicaId, replicaHealthReport, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    if (replicaHealthReport !== null && replicaHealthReport !== undefined) {
      let requestModelMapper = new client.models['ReplicaHealthReport']().mapper();
      requestModel = client.serialize(requestModelMapper, replicaHealthReport, 'replicaHealthReport');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(replicaHealthReport, null, 2)}.`);
...
/** Class representing a ReplicaHealths. */
class ReplicaHealths {
  /**
   * Create a ReplicaHealths.
   ...
   */
  getWithHttpOperationResponse(partitionId, replicaId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(partitionId, replicaId, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(partitionId, replicaId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._get(partitionId, replicaId, options, optionalCallback);
    }
  }
   ...
   * @reject {Error} - The error object.
   */
  sendWithHttpOperationResponse(partitionId, replicaId, replicaHealthReport, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._send(partitionId, replicaId, replicaHealthReport, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  send(partitionId, replicaId, replicaHealthReport, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._send(partitionId, replicaId, replicaHealthReport, options, optionalCallback);
    }
  }
