...
var DatabaseOperations = ( /** @lends DatabaseOperations */ function() {
   ...
   * which the database is hosted.
   * 
   * @param {string} databaseName The name of the Azure SQL Database.
   * 
   * @param {function} callback
   * 
   * @returns {Stream} The response stream.
   */
  DatabaseOperations.prototype.getUsages = function(serverName, databaseName, callback) {
    if (callback === null || callback === undefined) {
      throw new Error('callback cannot be null.');
    }
    // Validate
    ...
    return this.client.pipeline(httpRequest, function (err, response, body) {
      ...
      if (statusCode === 200) {
        ...
        xml.parseString(responseContent, options, function (err2, responseDoc) {
          ...
          var usagesSequenceElement = js2xml.getElement(responseDoc, responseDoc, 'USAGES', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
          if (usagesSequenceElement !== null && usagesSequenceElement !== undefined) {
            for (var loweredIndex1 = 0; loweredIndex1 < js2xml.getElements(responseDoc, usagesSequenceElement, 'USAGE', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE').length; loweredIndex1 = loweredIndex1 + 1) {
              var usagesElement = js2xml.getElements(responseDoc, usagesSequenceElement, 'USAGE', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE')[loweredIndex1];
              var usageInstance = {};
              result.usages.push(usageInstance);
              
              var nameElement = js2xml.getElement(responseDoc, usagesElement, 'NAME', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
              if (nameElement !== null && nameElement !== undefined) {
                var nameInstance = nameElement;
                usageInstance.name = nameInstance;
              }
              
              var resourceNameElement = js2xml.getElement(responseDoc, usagesElement, 'RESOURCENAME', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
              ...
              var currentValueElement = js2xml.getElement(responseDoc, usagesElement, 'CURRENTVALUE', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
              ...
              var limitElement = js2xml.getElement(responseDoc, usagesElement, 'LIMIT', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
              if (limitElement !== null && limitElement !== undefined) {
                var limitInstance = limitElement;
                usageInstance.limit = limitInstance;
              }
              
              var unitElement = js2xml.getElement(responseDoc, usagesElement, 'UNIT', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
              if (unitElement !== null && unitElement !== undefined) {
                var unitInstance = unitElement;
                usageInstance.unit = unitInstance;
              }
              
              var nextResetTimeElement = js2xml.getElement(responseDoc, usagesElement, 'NEXTRESETTIME', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
