 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _listWithCreateProjectPermission(options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/listBillingAccountsWithCreateProjectPermission ';
  let queryParameters = [];
  queryParameters.push('api-version=' + encodeURIComponent(this.client.apiVersion));
  if (expand !== null && expand !== undefined) {
    queryParameters.push('$expand=' + encodeURIComponent(expand));
...
function _get(billingAccountId, options, callback) {
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/billingAccounts/{billingAccountId}';
...
class BillingAccounts {
  ...
  constructor(client) {
    this.client = client;
    this._list = _list;
    this._listWithCreateProjectPermission = _listWithCreateProjectPermission;
    this._get = _get;
  }
   ...
   * @reject {Error} - The error object.
   */
  listWithCreateProjectPermissionWithHttpOperationResponse(options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._listWithCreateProjectPermission(options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  listWithCreateProjectPermission(options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._listWithCreateProjectPermission(options, optionalCallback);
    }
  }
