 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _get(billingAccountId, departmentId, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/billingAccounts/{billingAccountId}/departments/{departmentId}';
  requestUrl = requestUrl.replace('{billingAccountId}', encodeURIComponent(billingAccountId));
...
class Departments {
   ...
   * @reject {Error} - The error object.
   */
  getWithHttpOperationResponse(billingAccountId, departmentId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(billingAccountId, departmentId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(billingAccountId, departmentId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._get(billingAccountId, departmentId, options, optionalCallback);
    }
  }
