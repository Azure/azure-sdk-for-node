...
function _createProject(name, options, callback) {
  ...
  if (targetExportPlatforms !== null && targetExportPlatforms !== undefined) {
    if (targetExportPlatforms.length == 0) {
      queryParameters.push('targetExportPlatforms=' + encodeURIComponent(''));
    } else {
      for (let item of targetExportPlatforms) {
        item = (item === null || item === undefined) ? '' : item;
        queryParameters.push('targetExportPlatforms=' + encodeURIComponent('' + item));
      }
    }
  }
  if (queryParameters.length > 0) {
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _publishIteration(projectId, iterationId, name, predictionId, options, callback) {
   /* jshint validthis: true */
  let client = this;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class TrainingAPIClient extends ServiceClient {
   ...
   * @reject {Error} - The error object.
   */
  publishIterationWithHttpOperationResponse(projectId, iterationId, name, predictionId, options) {
    let client = this;
    let self = this;
    return new Promise((resolve, reject) => {
      self._publishIteration(projectId, iterationId, name, predictionId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  publishIteration(projectId, iterationId, name, predictionId, options, optionalCallback) {
    let client = this;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._publishIteration(projectId, iterationId, name, predictionId, options, optionalCallback);
    }
  }
