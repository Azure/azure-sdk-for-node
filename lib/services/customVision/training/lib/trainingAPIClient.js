...
function _trainProject(projectId, options, callback) {
  ...
  let reservedBudgetInHours = (options && options.reservedBudgetInHours !== undefined) ? options.reservedBudgetInHours : undefined;
  let forceTrain = (options && options.forceTrain !== undefined) ? options.forceTrain : undefined;
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _publishIteration(projectId, iterationId, name, options, callback) {
   /* jshint validthis: true */
  let client = this;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class TrainingAPIClient extends ServiceClient {
   ...
   * @reject {Error} - The error object.
   */
  publishIterationWithHttpOperationResponse(projectId, iterationId, name, options) {
    let client = this;
    let self = this;
    return new Promise((resolve, reject) => {
      self._publishIteration(projectId, iterationId, name, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  publishIteration(projectId, iterationId, name, options, optionalCallback) {
    let client = this;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._publishIteration(projectId, iterationId, name, options, optionalCallback);
    }
  }
