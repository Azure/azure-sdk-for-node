...
var EnvironmentVariables = ServiceClient.EnvironmentVariables;
...
/*
* Dealing with sdk configuration object
*/
...
// Do we have the full set of old environment variables?
...
function createSettingsFromEnvironment() {
  ...
  var hostname = namespace + '.' + ServiceClient.CLOUD_SERVICEBUS_HOST;
  ...
  if (!acsNamespace) {
    acsNamespace = namespace + ServiceClient.DEFAULT_WRAP_NAMESPACE_SUFFIX;
  }
  var acshostname = acsNamespace + '.' + ServiceClient.CLOUD_ACCESS_CONTROL_HOST;
...
ServiceBusSettings.customizeConfig = function (config) {
  function serviceBus(connectionStringOrOptions) {
    if (_.isString(connectionStringOrOptions)) {
      ...
      var host = connectionStringOrOptions.host || ServiceClient.CLOUD_SERVICEBUS_HOST;
      var stshost = connectionStringOrOptions.acsHost || ServiceClient.CLOUD_ACCESS_CONTROL_HOST;
      var acsNamespace = connectionStringOrOptions.wrapNamespace ||
        namespace + ServiceClient.DEFAULT_WRAP_NAMESPACE_SUFFIX;

      var hostname = namespace + '.' + host;
      var stshostname = acsNamespace + '.' + stshost;
      var issuer = connectionStringOrOptions.issuer || ServiceClient.DEFAULT_SERVICEBUS_ISSUER;
      var key = connectionStringOrOptions.key;

      var parts = {
        Endpoint: 'sb://' + namespace + '.' + hostname + ':443',
        StsEndpoint: 'https://' + acsNamespace + '.' + stshostname + ':443/WRAPv0.9',
        SharedSecretIssuer: issuer,
