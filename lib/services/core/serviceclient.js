...
ServiceClient.CLOUD_SERVICE_MANAGEMENT_HOST = 'management-preview.core.windows-int.net';
  ...
  if (!azureutil.objectIsNull(response.body) && response.body.trim() !== '') {
    if (response.headers['content-type'].indexOf('application/json') != -1) {
      ...
      try {
        ...
        parsedJson = { parsingError: error };
      }
      if (!azureutil.objectIsNull(parsedJson.parsingError)) {
        response.isSuccessful = false;
        response.parsingError = parsedJson.parsingError;
      } else if (!azureutil.objectIsNull(parsedJson)) {
