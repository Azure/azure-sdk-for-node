...
function _identify(personGroupId, faceIds, options, callback) {
  ...
  try {
    ...
    if (confidenceThreshold !== null && confidenceThreshold !== undefined) {
      ...
      {
        throw new Error('"confidenceThreshold" should satisfy the constraint - "InclusiveMaximum": 1');
      }
      if (confidenceThreshold < 0)
      {
        throw new Error('"confidenceThreshold" should satisfy the constraint - "InclusiveMinimum": 0');
      }
    }
  } catch (error) {
    return callback(error);
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _verify(faceId, personId, personGroupId, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
function _detect(url, options, callback) {
  ...
  try {
    ...
    if (returnFaceAttributes !== null && returnFaceAttributes !== undefined && typeof returnFaceAttributes.valueOf() !== 'string') {
      throw new Error('returnFaceAttributes must be of type string.');
    }
  ...
  if (returnFaceAttributes !== null && returnFaceAttributes !== undefined) {
    queryParameters.push('returnFaceAttributes=' + encodeURIComponent(returnFaceAttributes));
  }
...
class Face {
   ...
   * @reject {Error} - The error object.
   */
  verifyWithHttpOperationResponse(faceId, personId, personGroupId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._verify(faceId, personId, personGroupId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  verify(faceId, personId, personGroupId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._verify(faceId, personId, personGroupId, options, optionalCallback);
    }
  }
