 ...
 */
function _listByVaults(resourceGroupName, vaultName, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'Subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.RecoveryServices/vaults/{vaultName}/usages';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['VaultUsageList']().mapper();
...
class Usages {
  ...
  constructor(client) {
    this.client = client;
    this._listByVaults = _listByVaults;
  }
   ...
   */
  listByVaultsWithHttpOperationResponse(resourceGroupName, vaultName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._listByVaults(resourceGroupName, vaultName, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  listByVaults(resourceGroupName, vaultName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._listByVaults(resourceGroupName, vaultName, options, optionalCallback);
    }
  }
