 ...
 */
function _getCapabilities(location, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'subscriptions/{subscriptionId}/providers/Microsoft.HDInsight/locations/{location}/capabilities';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['CapabilitiesResult']().mapper();
...
class Location {
  ...
  constructor(client) {
    this.client = client;
    this._getCapabilities = _getCapabilities;
  }
   ...
   */
  getCapabilitiesWithHttpOperationResponse(location, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getCapabilities(location, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getCapabilities(location, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._getCapabilities(location, options, optionalCallback);
    }
  }
