...
var RoleAssignmentOperations = ( /** @lends RoleAssignmentOperations */ function() {
   ...
   * 
   * @param {RoleAssignmentProperties} parameters.roleAssignment Role
   * assignment properties.
   * 
   * @param {string} [parameters.roleAssignment.roleDefinitionId] Gets or sets
   * role definition id.
   * 
   * @param {string} [parameters.roleAssignment.principalId] Gets or sets
   * principal Id.
   * 
   * @param {string} [parameters.roleAssignment.scope] Gets or sets role
   * assignment scope.
   * 
  ...
  RoleAssignmentOperations.prototype.create = function(scope, roleAssignmentName, parameters, callback) {
    ...
    // Construct URL
    var url2 = '/' + scope.trim() + '/providers/Microsoft.Authorization/roleAssignments/' + roleAssignmentName.trim() + '?';
    url2 = url2 + 'api-version=2014-05-21';
    ...
    if (parameters.roleAssignment.roleDefinitionId) {
      propertiesValue['roleDefinitionId'] = parameters.roleAssignment.roleDefinitionId;
    }
    
    if (parameters.roleAssignment.principalId) {
      propertiesValue['principalId'] = parameters.roleAssignment.principalId;
    }
    
    if (parameters.roleAssignment.scope) {
      propertiesValue['scope'] = parameters.roleAssignment.scope;
    }
   ...
   * 
   * @param {string} [parametersOrCallback.principalId] Returns role assignment
  ...
  RoleAssignmentOperations.prototype.list = function(parametersOrCallback, callback) {
    ...
    if (parameters && parameters.principalId) {
      url2 = url2 + '&$filter=principalId eq ' + encodeURIComponent(parameters.principalId ? parameters.principalId.trim() : '');
    }
    url2 = url2 + '&api-version=2014-05-21';
    var baseUrl = this.client.baseUri;
    // Trim '/' character from the end of baseUrl and beginning of url.
...
var RoleDefinitionOperations = ( /** @lends RoleDefinitionOperations */ function() {
   ...
   * 
   * @param {string} roleName Role definition name (GUID).
   * 
   * @param {function} callback
   * 
   * @returns {Stream} The response stream.
   */
  RoleDefinitionOperations.prototype.get = function(roleName, callback) {
    if (callback === null || callback === undefined) {
      throw new Error('callback cannot be null.');
    }
    // Validate
    ...
    // Construct URL
    var url2 = '/subscriptions/' + (this.client.credentials.subscriptionId ? this.client.credentials.subscriptionId.trim() : '') + '/providers/Microsoft.Authorization/roleDefinitions/' + roleName.trim() + '?';
    url2 = url2 + 'api-version=2014-05-21';
    var baseUrl = this.client.baseUri;
    // Trim '/' character from the end of baseUrl and beginning of url.
    if (baseUrl[baseUrl.length - 1] === '/') {
      ...
      if (responseDoc) {
        ...
        if (propertiesValue) {
          ...
          if (roleNameValue) {
            ...
            propertiesInstance.name = roleNameInstance;
          }
