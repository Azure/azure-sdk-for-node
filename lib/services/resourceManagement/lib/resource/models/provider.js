...
Provider.prototype.validate = function (payload) {
  ...
  if (payload['namespace'] !== null && payload['namespace'] !== undefined && typeof payload['namespace'] !== 'string') {
    throw new Error('payload[\'namespace\'] must be of type string.');
  }

  if (payload['registrationState'] !== null && payload['registrationState'] !== undefined && typeof payload['registrationState'] !== 'string') {
    throw new Error('payload[\'registrationState\'] must be of type string.');
  }

  if (payload['resourceTypes'] !== null && payload['resourceTypes'] !== undefined && util.isArray(payload['resourceTypes'])) {
    for (var i = 0; i < payload['resourceTypes'].length; i++) {
      if (payload['resourceTypes'][i] !== null && payload['resourceTypes'][i] !== undefined) {
