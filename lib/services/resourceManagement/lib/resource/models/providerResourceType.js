...
ProviderResourceType.prototype.validate = function (payload) {
  ...
  if (payload['locations'] !== null && payload['locations'] !== undefined && util.isArray(payload['locations'])) {
    for (var i = 0; i < payload['locations'].length; i++) {
      if (payload['locations'][i] !== null && payload['locations'][i] !== undefined && typeof payload['locations'][i] !== 'string') {
        throw new Error('payload[\'locations\'][i] must be of type string.');
      }
    }
  }

  if (payload['apiVersions'] !== null && payload['apiVersions'] !== undefined && util.isArray(payload['apiVersions'])) {
    for (var i1 = 0; i1 < payload['apiVersions'].length; i1++) {
      if (payload['apiVersions'][i1] !== null && payload['apiVersions'][i1] !== undefined && typeof payload['apiVersions'][i1] !== 'string') {
  ...
  if (payload['properties'] !== null && payload['properties'] !== undefined && typeof payload['properties'] === 'object') {
    for(var valueElement in payload['properties']) {
      if (payload['properties'][valueElement] !== null && payload['properties'][valueElement] !== undefined && typeof payload['properties'][valueElement] !== 'string') {
