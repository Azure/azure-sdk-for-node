...
ResourcesMoveInfo.prototype.validate = function (payload) {
  ...
  if (payload['resources'] !== null && payload['resources'] !== undefined && util.isArray(payload['resources'])) {
    for (var i = 0; i < payload['resources'].length; i++) {
      if (payload['resources'][i] !== null && payload['resources'][i] !== undefined && typeof payload['resources'][i] !== 'string') {
        throw new Error('payload[\'resources\'][i] must be of type string.');
      }
    }
  }

  if (payload['targetResourceGroup'] !== null && payload['targetResourceGroup'] !== undefined && typeof payload['targetResourceGroup'] !== 'string') {
