 ...
 *
 * @param {Object} [parameters.properties] Gets or sets the resource group properties.
 *
 * @param {String} [parameters.provisioningState] Gets or sets resource group provisioning state.
 *
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 201) {
      ...
      try {
        ...
        result.body = parsedResponse;
        if (result.body !== null && result.body !== undefined) {
          result.body = client._models['ResourceGroupExtended'].deserialize(result.body);
        }
      } catch (error) {
        var deserializationError = new Error(util.format('Error "%s" occurred in deserializing the responseBody - "%s"', error, responseBody));
        deserializationError.request = httpRequest;
 ...
 * @param {ResourceGroupExtendedFilter} [filter] The filter to apply on the operation.
 *
 * @param {String} [filter.tagName] Gets or sets the tag name.
 *
 * @param {String} [filter.tagValue] Gets or sets the tag value.
...
ResourceGroups.prototype.list = function (filter, top, options, callback) {
  ...
  try {
    if (filter !== null && filter !== undefined) {
      client._models['ResourceGroupExtendedFilter'].validate(filter);
    }
    if (top !== null && top !== undefined && typeof top !== 'number') {
      throw new Error('top must be of type number.');
    }
