...
BlobService.prototype.abortCopyBlob = function (container, blob, copyId, optionsOrCallback, callback) {
  ...
  var webResource = WebResource.put(resourceName)
    ...
    .addOptionalHeader(HeaderConstants.COPY_ACTION, 'abort')
...
BlobService.prototype._leaseBlobImpl = function (container, blob, leaseId, leaseAction, optionsOrCallback, callback) {
  ...
  var webResource = WebResource.put(resourceName)
    ...
    .addOptionalHeader(HeaderConstants.LEASE_ID_HEADER, leaseId)
    .addOptionalHeader(HeaderConstants.LEASE_ACTION_HEADER, leaseAction.toLowerCase());

  if (options) {
    if (options.leaseBreakPeriod) {
      webResource.addOptionalHeader(HeaderConstants.LEASE_BREAK_PERIOD, options.leaseBreakPeriod);
    }

    if (options.proposedLeaseId) {
      webResource.addOptionalHeader(HeaderConstants.PROPOSED_LEASE_ID, options.proposedLeaseId);
    }

    if (options.leaseDuration) {
      webResource.addOptionalHeader(HeaderConstants.LEASE_DURATION, options.leaseDuration);
    }
  }
