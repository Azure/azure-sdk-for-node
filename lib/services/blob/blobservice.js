...
/**
* Create unencoded resource name (needed for Shared Access Signatures)
* @ignore
*
* @param {string} containerName Container name
* @param {string} blobName      Blob name
* @return {string} The unencoded resource name.
*/
function createUnencodedResourceName(containerName, blobName) {
  var resourceName = containerName + '/' + blobName;
  if (!containerName || containerName === '$root') {
    resourceName = blobName;
  }
  ...
  // return unencoded resource name
  return resourceName;
}
...
BlobService.prototype.generateSharedAccessSignature = function (container, blob, sharedAccessPolicy) {
  ...
  var resourceName = createResourceName(container, blob);
  var unencodedResourceName = createUnencodedResourceName(container, blob);
  var signedQueryString = this.sharedAccessSignatureCredentials.generateSignedQueryString(unencodedResourceName, {}, resourceType, sharedAccessPolicy);
