...
BlobService.prototype.createPageBlobFromFile = function (container, blob, localFileName, optionsOrCallback, callback) {
  var options, _callback;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { options = o; _callback = c; });
  var self = this;
  fs.stat(localFileName, function(error, stat) {
    if (error) {
      _callback(error, null);
    } else {
      self.createPageBlobFromStream(container, blob, localFileName, stat.size, optionsOrCallback, callback);
    }
  });
};
...
BlobService.prototype.createPageBlobFromStream = function(container, blob, streamOrPath, streamLength, optionsOrCallback, callback) {
  if (streamLength % Constants.BlobConstants.PAGE_SIZE == 0) {
    ...
    throw new Error(util.format('The page blob size must be aligned to a 512-byte boundary. The current stream length is %s', streamLength));
  }
}
...
BlobService.prototype._createBlobFromStream = function(container, blob, blobType, streamOrPath, optionsOrCallback, callback) {
  ...
  if (blobType == BlobConstants.BlobTypes.BLOCK) {
    ...
    sizeLimitation = Constants.BlobConstants.DEFAULT_MINIMUM_PAGE_STREAM_WRITE_IN_BYTES;
  } else {
    throw new Error(util.format('Unknown blob type %s', blobType));
  }
  ...
  fileStream.on('data', function(data, range) {
    ...
    if(data.length > sizeLimitation) {
      throw new Error(util.format('Upload single block/page exceed the size limitation. Max is %s and the current data is %s', sizeLimitation, data.length));
    }
    ...
    if (isBlockBlobUpload) {
      ...
      operation = new BatchOperation.RestOperation(self, apiName, blockId, container, blob, data, rangeOptions);
    } else {
      ...
      if (azureutil.isBufferAllZero(data)) {
        ...
        operation = new BatchOperation.RestOperation(self, apiName, container, blob, data, range.start, range.end, rangeOptions);
      }
    }
