...
BlobService.prototype.generateBlockIdPrefix = function() {
  prefix = parseInt(Math.random() * Math.pow(10,8)).toString(16);
  return azureutil.zeroPaddingString(prefix, 8);
}

BlobService.prototype.getBlockId = function(prefix, id) {
  return  prefix + '-' + azureutil.zeroPaddingString(id, 6);
}
...
BlobService.prototype._getBlobToFile = function(container, blob, blobType, localFileName, optionsOrCallback, callback) {
  ...
  rangeStream.on('range', function(range) {
    if (range.type != Constants.UNCOMMITTED_BLOCKS_ELEMENT) {
      ...
      if (range.start != writeOffset) {
        ...
        self.logger.debug(util.format('Write Zero from %s to %s', writeOffset, range.start));
  ...
  //TODO use the following source code
...
BlobService.prototype.createPageBlobFromFile = function (container, blob, localFileName, optionsOrCallback, callback) {
  ...
  fs.stat(localFileName, function(error, stat) {
    if (error) {
      ...
      self.createPageBlobFromStream(container, blob, localFileName, optionsOrCallback, callback);
    }
  })
};
...
BlobService.prototype._createBlobFromStream = function(container, blob, blobType, streamOrPath, optionsOrCallback, callback) {
  ...
  fileStream.on('data', function(data) {
    ...
    if (isBlock) {
      ...
      operation = new BatchOperation.RestOperation(self, apiName, blockId, container, blob, data, options);
    } else {
      apiName = 'createBlobPagesFromText';
      range.end = range.start + data.length - 1;
      //TODO move range start and range end to fileStream
      debugger;
      if (azureutil.isBufferAllZero(data)) {
        self.logger.debug(util.format('Skip upload data from %s to %s to blob %s', range.start, range.end, blob));
      } else {
  ...
  batchOperations.on('end', function(error, retValue) {
    if (isBlock) {
      ...
      self.commitBlobBlocks(container, blob, blockList, function(error, retValue) {
        if(error) throw error;
