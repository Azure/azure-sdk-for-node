    ...
    if (range.type != Constants.UNCOMMITTED_BLOCKS_ELEMENT) {
      ...
      if (range.start != writeOffset) {
        if (!isBlockBlobDownload) {
          var writeZeroOperation = new BatchOperation.CommonOperation(azureutil.writeZeroToStream, writeStream, range.start - writeOffset, md5Hash);
...
BlobService.prototype.createPageBlobFromStream = function(container, blob, streamOrPath, streamLength, optionsOrCallback, callback) {
  ...
  if (streamLength % Constants.BlobConstants.PAGE_SIZE === 0) {
    var self = this;
    this.createPageBlob(container, blob, streamLength, options, function(error, retValue) {
      if(error) {
        callback(error, null);
      } else {
        self.clearBlobPages(container, blob, 0, streamLength - 1, options, function(error, retValue) {
          if(error) {
            callback(error, null);  
          } else {
