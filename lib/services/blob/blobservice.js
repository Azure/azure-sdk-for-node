...
var BatchOperation = require('./models/batchoperation');
var SpeedSummary = require('./models/speedsummary');
var ChunkAllocator = require('./models/chunkAllocator');
var ChunkStream = require('./models/chunkStream');
...
BlobService.prototype.createBlockBlobFromStream = function (container, blob, stream, streamLength, optionsOrCallback, callback) {
  if (streamLength >= this.singleBlobPutThresholdInBytes) {
    ...
    return this.putBlockBlobFromStream(container, blob, stream, streamLength, optionsOrCallback, callback);
  }
};
...
BlobService.prototype._getBlobToFile = function(container, blob, blobType, localFileName, optionsOrCallback, callback) {
  ...
  rangeStream.on('range', function(range) {
    ...
    if (range.type != Constants.UNCOMMITTED_BLOCKS_ELEMENT) {
      ...
      if (range.size === 0) { //range.size is data size instead of range size
        if (!isBlockBlobDownload) {
          var autoIncrement = speedSummary.getAutoIncrementFunction(rangeSize);
          //Use stamp function to wait for write zero to file
          var writeZeroOperation = new BatchOperation.CommonOperation(BatchOperation.stampFunction, function() {
            azureutil.writeZeroToStream(writeStream, rangeSize, md5Hash, autoIncrement);
            //There is no need to pause the rangestream since we can perform http request and write disk at the same time
            self.logger.debug(util.format('Write Zero from %s to %s', range.start, range.end));
          });
      ...
      if(range.start > range.end || range.size ===0) {
        return;
      }
...
BlobService.prototype._createBlobFromStream = function(container, blob, blobType, stream, streamLength, optionsOrCallback, callback) {
  ...
  readOptions.allocator = allocator;
  if(stream.setMemoryAllocator) {
    stream.setMemoryAllocator(allocator);
  }

  var chunkStream = new ChunkStream(stream, readOptions);
  ...
  chunkStream.on('data', function(data, range) {
    ...
    if (isBlockBlobUpload) {
      ...
      operation = new BatchOperation.RestOperation(self, apiName, blockId, container, blob, data, rangeOptions, function() {
        autoIncrement();
      ...
      if (azureutil.isBufferAllZero(data)) {
        ...
        operation = new BatchOperation.RestOperation(self, apiName, container, blob, data, range.start, range.end, rangeOptions, function() {
          autoIncrement();
          allocator.releaseBuffer(data);
          data = null;
        });
  ...
  batchOperations.on('end', function(error) {
    self.logger.debug('batch operations ended');
    speedSummary = null;
    if (error) {
      callback(error);
      return;
...
/**
 * A blob container
 * @typedef {object} BlobService~containerResult
 * @property {string} etag
 * @property {string} lastModified
 * @property {string} leaseStatus
 * @property {string} leaseState
 * @property {string} leaseDuration
