...
BlobService.prototype.getBlobToFile = function (container, blob, localFilename, optionsOrCallback, callback) {
  ...
  //FIXME check the options and callback
  var self = this;
  this.getBlobProperties(container, blob, propertiesRequestOptions, function(error, properties) {
    if (error) {
      callback(error, null);  
    } else {
      self._getBlobToFile(container, blob, properties.blobType, localFilename, optionsOrCallback, callback);
    } 
  }); 
  ...
  fs.stat(localFilename, function(error, stat) {
    if (error) {
      ...
      if (stat.size < self.singleBlobPutThresholdInBytes) {
        self.putBlockBlobFromFile(container, blob, localFilename, optionsOrCallback, callback);
      } else {
        self.createBlockBlobFromStream(container, blob, localFilename, -1, optionsOrCallback, callback);
      }
    }
  });
}
...
BlobService.prototype.createBlockBlobFromStream = function (container, blob, streamOrPath, streamLength, optionsOrCallback, callback) {
  this._createBlobFromStream(container, blob, BlobConstants.BlobTypes.BLOCK, streamOrPath, optionsOrCallback, callback); 
}
...
BlobService.prototype._getBlobToFile = function(container, blob, blobType, localFileName, optionsOrCallback, callback) {
  ...
  if (blobType == BlobConstants.BlobTypes.PAGE) {
    ...
    throw new Error(util.format('Unknown blob type %s', blobType));  
  } 
  
  var writeStream = fs.createWriteStream(localFileName);
  var batchOperations = new BatchOperation('getblob', {callbackInorder: true});
  BatchOperation.setConcurrency(options.ParallelOperationThreadCount);
  var self = this;
    ...
    if (range.type != Constants.UNCOMMITTED_BLOCKS_ELEMENT) {
      ...
      if (range.start != writeOffset) {
        if (!isBlockBlobDownload) {
          var writeZeroOperation = new BatchOperation.CommonOperation(azureutil.writeZeroToStream, writeStream, range.start - writeOffset);  
      ...
      if(full) {
        self.logger.debug('Stream pause');
        rangeStream.pause();  
      }
    }
  });
...
* @param {string}             container                                   The container name.
* @param {string}             blob                                        The blob name.
* @param (string)             localFileName                               File path to the data to store.
* @param {object|function}    [optionsOrCallback]                         The creating and request options.
* @param {string}             [optionsOrCallback.blockIdPrefix]           The prefix for block id.
* @param {string}             [optionsOrCallback.snapshotId]              The snapshot identifier.
* @param {string}             [optionsOrCallback.leaseId]                 The lease identifier.
...
BlobService.prototype.createPageBlobFromFile = function (container, blob, localFileName, optionsOrCallback, callback) {
  this.createPageBlobFromStream(container, blob, localFileName, optionsOrCallback, callback);
};
...
* @return {undefined}
*/
BlobService.prototype.createPageBlobFromStream = function(container, blob, streamOrPath, optionsOrCallback, callback) {
  this._createBlobFromStream(container, blob, BlobConstants.BlobTypes.PAGE, streamOrPath, optionsOrCallback, callback); 
}
...
BlobService.prototype._createBlobFromStream = function(container, blob, blobType, streamOrPath, optionsOrCallback, callback) {
  var options;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { options = o; callback = c; });
  ...
  var batchOperations = new BatchOperation(operationName);
  var blockMd5Recorder = {};
  var blockCount = 0;
  var blockIds = [];
  var self = this;
  BatchOperation.setConcurrency(options.ParallelOperationThreadCount);
  ...
  fileStream.on('data', function(data, range) {
    ...
    if (isBlockBlobUpload) {
      var saveId = blockMd5Recorder[contentMD5];
      if (saveId && saveId.length) {
        //Find a same content has been uploaded successfully and there is no need to upload again 
        self.logger.debug(util.format('Reuse block %s instead of uploading a new one', saveId));
        blockIds.push(saveId);
      } else {
        var blockId = self.getBlockId(blockIdPrefix, blockCount); 
        blockIds.push(blockId);
        apiName = 'createBlobBlockFromText';
        operation = new BatchOperation.RestOperation(self, apiName, blockId, container, blob, data, options, function(error, retValue) {
          if(!error) {
            blockMd5Recorder[contentMD5] = blockId; 
          }
          blockId = null;
        });
      }
      ...
      if (azureutil.isBufferAllZero(data)) {
        ...
        operation = new BatchOperation.RestOperation(self, apiName, container, blob, data, range.start, range.end, options); 
      }
    }
  ...
  batchOperations.on('end', function(error, retValue) {
    if (isBlockBlobUpload) {
      ...
      var commitOptions = {};
      commitOptions.contentMD5Header = fileStream.getContentMd5();
      self.commitBlobBlocks(container, blob, blockList, commitOptions, function(error, retValue) {
