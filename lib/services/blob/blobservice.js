...
BlobService.prototype.copyBlob = function (sourceContainer, sourceBlob, targetContainer, targetBlob, optionsOrCallback, callback) {
  ...
  var sourceAccount = this.storageAccount;
  ...
  try {
    if (options) {
      ...
      if (options.sourceAccount) {
        ...
        this.apiVersion = max(this.apiVersion, '2012-02-12');
        sourceAccount = options.sourceAccount;
        webResource.withOkCode(HttpConstants.HttpResponseCodes.ACCEPTED_CODE);
        // Dogfood and production cluster are currently using different incompatible verison of this api
        // TODO this 'if' should be removed (become 'if (true)') when dogfood cluster switch to stg38 version or before shipping 
        if (!(process.env.blobapistage <= 37)) {
          prefix = 'http://';
          sourceAccount = options.sourceAccount + '.' + this.host;
        }
      }
    }
  
    webResource.addOptionalHeader(HeaderConstants.COPY_SOURCE_HEADER, prefix + sourceAccount + '/' + sourceResourceName);
...
BlobService.prototype.copyBlobEx = function (sourceUri, targetContainer, targetBlob, optionsOrCallback, callback) {
  ...
  try {
    ...
    webResource.withOkCode(HttpConstants.HttpResponseCodes.ACCEPTED_CODE);
    // Dogfood and production cluster are currently using different incompatible verison of this api
    // TODO this 'if' should be removeds when dogfood cluster switch to stg38 version or before shipping
    if (process.env.blobapistage <= 37) {
      var removeProtocol = sourceUri.split('://', 2).slice(-1)[0];
      sourceUri = '/' + removeProtocol.split('.', 1)[0] + '/' + removeProtocol.split('/').slice(1).join('/');
    }
