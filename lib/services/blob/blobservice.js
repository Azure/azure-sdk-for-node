...
var FileReadStream = require('./filereadstream');
...
BlobService.prototype.createBlockBlobFromFile = function (container, blob, localFilename, optionsOrCallback, callback) {
  ...
  fs.stat(localFileName, function(error, stat) {
    if (error) {
      ...
      if (stat.size < self.singleBlobPutThresholdInBytes / 8) {
...
BlobService.prototype.generateBlockId = function(prefix, blockId) {
  if (!prefix) {
    prefix = parseInt(Math.random() * Math.pow(10, 8)).toString(18);
  }
}
