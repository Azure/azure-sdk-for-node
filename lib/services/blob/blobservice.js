...
var ThroughChunkStream = require('./internal/throughChunkStream');
...
BlobService.prototype.createBlob = function (container, blob, blobType, optionsOrCallback, callback) {
  ...
  var stream = new ThroughChunkStream(readOptions);
  this._createBlobFromChunkStream(container, blob, blobType, stream, null, options, function (err) {
    if (callback) {
      callback(err);
    } else if (err) {
      throw err;
