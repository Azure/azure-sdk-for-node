...
function _list(resourceGroupName, serviceName, groupId, options, callback) {
  ...
  try {
    ...
    if (groupId !== null && groupId !== undefined) {
      ...
      {
        throw new Error('"groupId" should satisfy the constraint - "MaxLength": 80');
      }
      if (groupId.length < 1)
      {
        throw new Error('"groupId" should satisfy the constraint - "MinLength": 1');
      }
      if (groupId.match(/(^[\w]+$)|(^[\w][\w\-]+[\w]$)/) === null)
      {
        throw new Error('"groupId" should satisfy the constraint - "Pattern": /(^[\w]+$)|(^[\w][\w\-]+[\w]$)/');
      }
    }
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _checkEntityExists(resourceGroupName, serviceName, groupId, uid, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class GroupUser {
   ...
   * @reject {Error} - The error object.
   */
  checkEntityExistsWithHttpOperationResponse(resourceGroupName, serviceName, groupId, uid, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._checkEntityExists(resourceGroupName, serviceName, groupId, uid, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  checkEntityExists(resourceGroupName, serviceName, groupId, uid, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._checkEntityExists(resourceGroupName, serviceName, groupId, uid, options, optionalCallback);
    }
  }
