 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _updateTags(resourceGroupName, hanaInstanceName, hanaInstancePatchTagsParameter, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    ...
    if (hanaInstancePatchTagsParameter === null || hanaInstancePatchTagsParameter === undefined) {
      throw new Error('hanaInstancePatchTagsParameter cannot be null or undefined.');
    }
    if (this.client.acceptLanguage !== null && this.client.acceptLanguage !== undefined && typeof this.client.acceptLanguage.valueOf() !== 'string') {
      throw new Error('this.client.acceptLanguage must be of type string.');
    }
  ...
  try {
    if (hanaInstancePatchTagsParameter !== null && hanaInstancePatchTagsParameter !== undefined) {
      ...
      requestModel = client.serialize(requestModelMapper, hanaInstancePatchTagsParameter, 'hanaInstancePatchTagsParameter');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(hanaInstancePatchTagsParameter, null, 2)}.`);
...
class HanaInstances {
   ...
   * @reject {Error} - The error object.
   */
  updateTagsWithHttpOperationResponse(resourceGroupName, hanaInstanceName, hanaInstancePatchTagsParameter, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._updateTags(resourceGroupName, hanaInstanceName, hanaInstancePatchTagsParameter, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  updateTags(resourceGroupName, hanaInstanceName, hanaInstancePatchTagsParameter, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._updateTags(resourceGroupName, hanaInstanceName, hanaInstancePatchTagsParameter, options, optionalCallback);
    }
  }
