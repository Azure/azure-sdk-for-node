 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _configureRepo(resourceGroupName, factoryName, factoryRepoUpdate, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class Factories {
  ...
  constructor(client) {
    ...
    this._configureRepo = _configureRepo;
   ...
   * @reject {Error} - The error object.
   */
  configureRepoWithHttpOperationResponse(resourceGroupName, factoryName, factoryRepoUpdate, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._configureRepo(resourceGroupName, factoryName, factoryRepoUpdate, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  configureRepo(resourceGroupName, factoryName, factoryRepoUpdate, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._configureRepo(resourceGroupName, factoryName, factoryRepoUpdate, options, optionalCallback);
    }
  }
