 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _setCertificateContacts(vaultBaseUrl, options, callback) {
   /* jshint validthis: true */
  let client = this;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  let contactList = (options && options.contactList !== undefined) ? options.contactList : undefined;
  // Validate
  try {
    if (vaultBaseUrl === null || vaultBaseUrl === undefined || typeof vaultBaseUrl.valueOf() !== 'string') {
      throw new Error('vaultBaseUrl cannot be null or undefined and it must be of type string.');
  ...
  let contacts = new client.models['Contacts']();
  try {
...
class KeyVaultClient extends ServiceClient {
  ...
  setCertificateContactsWithHttpOperationResponse(vaultBaseUrl, options) {
    ...
    let self = this;
    return new Promise((resolve, reject) => {
      self._setCertificateContacts(vaultBaseUrl, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  setCertificateContacts(vaultBaseUrl, options, optionalCallback) {
    let client = this;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._setCertificateContacts(vaultBaseUrl, options, optionalCallback);
    }
  }
