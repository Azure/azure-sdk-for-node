...
import { ApplicationInsightsDataClient, ApplicationInsightsDataModels, ApplicationInsightsDataMappers } from "./lib/applicationInsightsDataClient";
...
msRestNodeAuth.loginWithServicePrincipalSecretWithAuthResponse(
  ...
  '72f988bf-86f1-41af-91ab-2d7cd011db47').then(
  (authRes) => {
    if (err) throw err
    // ..use the client instance to access service resources.
    const client = new ApplicationInsightsDataClient(authRes.credentials);
