...
var StorageAccountOperations = ( /** @lends StorageAccountOperations */ function() {
   ...
   * 
   * @param {string} parameters.name Required. A name for the storage account
   * that is unique within Windows Azure. Storage account names must be
    ...
    return this.client.pipeline(httpRequest, function (err, response, body) {
      ...
      return xml.parseString(responseContent, options, function (err2, responseDoc) {
        ...
        if (storageServicesSequenceElement) {
          for (var loweredIndex1 = 0; loweredIndex1 < js2xml.getElements(responseDoc, storageServicesSequenceElement, 'STORAGEACCOUNT', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE').length; loweredIndex1 = loweredIndex1 + 1) {
            var storageServicesElement = js2xml.getElements(responseDoc, storageServicesSequenceElement, 'STORAGEACCOUNT', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE')[loweredIndex1];
            var storageAccountInstance = { extendedProperties: {} };
            result.storageAccounts.push(storageAccountInstance);
            ...
            if (extendedPropertiesSequenceElement) {
              for (var loweredIndex3 = 0; loweredIndex3 < js2xml.getElements(responseDoc, extendedPropertiesSequenceElement, 'EXTENDEDPROPERTY', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE').length; loweredIndex3 = loweredIndex3 + 1) {
                ...
                var extendedPropertiesKey = js2xml.getElement(responseDoc, extendedPropertiesElement, 'NAME', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
                var extendedPropertiesValue = js2xml.getElement(responseDoc, extendedPropertiesElement, 'VALUE', 'HTTP://SCHEMAS.MICROSOFT.COM/WINDOWSAZURE');
                storageAccountInstance.extendedProperties[extendedPropertiesKey] = extendedPropertiesValue;
              }
            }
          }
        }
