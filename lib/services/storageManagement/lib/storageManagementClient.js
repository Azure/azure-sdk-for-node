...
var StorageAccountOperations = ( /** @lends StorageAccountOperations */ function() {
   ...
   * 
   * @param {string} parameters.name A name for the storage account that is
   * unique within Windows Azure. Storage account names must be between 3 and
   ...
   * as abase64-encoded string. The name may be up to 100 characters inlength.
   * The name can be used identify the storage account for yourtracking
   * purposes.
   * 
   ...
   * storage account is created with the geo-replication enabled. If the
   * element is not included in the request body, the default value is true.
   * If set to true, the data in the storage account is replicated across more
   * than one geographic location so as to enable resilience in the face of
   * catastrophic service loss.
   * 
   * @param {object} [parameters.extendedProperties] Represents the name of an
   * extended storage account property. Each extended property must have both
   * a defined name and value. You can have a maximum of 50 extended property
  ...
  StorageAccountOperations.prototype.beginCreating = function(parameters, callback) {
    ...
    var url2 = '/' + this.client.credentials.subscriptionId.trim() + '/services/storageservices';
    // Trim '/' character from the end of baseUrl and beginning of url.
    if (baseUrl[baseUrl.length - 1] === '/') {
      baseUrl = baseUrl.substring(0, (baseUrl.length - 1) + 0);
    }
  ...
  StorageAccountOperations.prototype.checkNameAvailability = function(accountName, callback) {
    ...
    // Construct URL
    var baseUrl = this.client.baseUri;
    var url2 = '/' + this.client.credentials.subscriptionId.trim() + '/services/storageservices/operations/isavailable/' + accountName.trim();
    // Trim '/' character from the end of baseUrl and beginning of url.
    if (baseUrl[baseUrl.length - 1] === '/') {
      baseUrl = baseUrl.substring(0, (baseUrl.length - 1) + 0);
    }
  ...
  StorageAccountOperations.prototype.delete = function(accountName, callback) {
    ...
    // Construct URL
    var baseUrl = this.client.baseUri;
    var url2 = '/' + this.client.credentials.subscriptionId.trim() + '/services/storageservices/' + accountName.trim();
    // Trim '/' character from the end of baseUrl and beginning of url.
    if (baseUrl[baseUrl.length - 1] === '/') {
      baseUrl = baseUrl.substring(0, (baseUrl.length - 1) + 0);
    }
   ...
   * more information)
   * 
   * @param {string} accountName Name of the storage account to get.
   * 
   * @param {function} callback
   * 
   * @returns {Stream} The response stream.
  ...
  StorageAccountOperations.prototype.getKeys = function(accountName, callback) {
    ...
    // Construct URL
    var baseUrl = this.client.baseUri;
    var url2 = '/' + this.client.credentials.subscriptionId.trim() + '/services/storageservices/' + accountName.trim() + '/keys';
    // Trim '/' character from the end of baseUrl and beginning of url.
    if (baseUrl[baseUrl.length - 1] === '/') {
      baseUrl = baseUrl.substring(0, (baseUrl.length - 1) + 0);
    }
  ...
  StorageAccountOperations.prototype.regenerateKeys = function(parameters, callback) {
    ...
    // Construct URL
    var baseUrl = this.client.baseUri;
    var url2 = '/' + this.client.credentials.subscriptionId.trim() + '/services/storageservices/' + parameters.name.trim() + '/keys?action=regenerate';
    // Trim '/' character from the end of baseUrl and beginning of url.
    if (baseUrl[baseUrl.length - 1] === '/') {
      baseUrl = baseUrl.substring(0, (baseUrl.length - 1) + 0);
    }
   ...
   * 
   * @param {string} [parameters.label] A name for the storage account that is
   * base64-encoded. The label may be up to 100 characters in length. The name
   * can be used identify the storage account for your tracking purposes.
   * 
   * @param {boolean} [parameters.geoReplicationEnabled] Enables or disables
   * geo-replication on the specified the storage. If set to true, the data in
   * the storage account is replicated across more than one geographic
   * location so as to enable resilience in the face of catastrophic service
   * loss. If set to false, geo-replication is disabled. If the element is not
