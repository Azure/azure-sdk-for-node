 ...
 */
function _listByType(automationAccountName, moduleName, typeName, options, callback) {
  ...
  try {
    ...
    if (typeName === null || typeName === undefined || typeof typeName.valueOf() !== 'string') {
      throw new Error('typeName cannot be null or undefined and it must be of type string.');
    }
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Automation/automationAccounts/{automationAccountName}/modules/{moduleName}/types/{typeName}/fields';
  ...
  requestUrl = requestUrl.replace('{typeName}', encodeURIComponent(typeName));
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['TypeFieldListResult']().mapper();
...
class Fields {
   ...
   */
  listByTypeWithHttpOperationResponse(automationAccountName, moduleName, typeName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._listByType(automationAccountName, moduleName, typeName, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  listByType(automationAccountName, moduleName, typeName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._listByType(automationAccountName, moduleName, typeName, options, optionalCallback);
    }
  }
