...
ServiceBusServiceClient.DEFAULT_ISSUER = 'owner';
ServiceBusServiceClient.WRAP_NAMESPACE_SUFFIX = '-sb';
...
* @param {object} authenticationProvider  The authentication provider object (e.g. wrap).
*/
function ServiceBusServiceClient(host, namespace, acsnamespace, issuer, password, authenticationProvider) {
  ...
  if (!this.acsnamespace) {
    ...
    if (!this.acsnamespace) {
      this.acsnamespace = this.namespace + ServiceBusServiceClient.WRAP_NAMESPACE_SUFFIX;
    }
  }
  ...
  if (!this.issuer) {
    ...
    if (!this.issuer) {
      // If still there is no issuer, just default
      this.issuer = ServiceBusServiceClient.DEFAULT_ISSUER;
    }
  }

  this.password = password;
  if (!this.password) {
    this.password = process.env[ServiceClient.EnvironmentVariables.AZURE_SERVICEBUS_PASSWORD]; ;
  }
  ...
  if (!this.authenticationProvider) {
    this.authenticationProvider = new Wrap(this.acsnamespace, this.issuer, this.password);
  }
}
