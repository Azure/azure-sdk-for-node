...
function _purge(resourceGroupName, resourceName, body, options, callback) {
  ...
  // Send request
  this.beginPurge(resourceGroupName, resourceName, body, options, (err, parsedResult, httpRequest, response) => {
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _beginPurge(resourceGroupName, resourceName, body, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class Components {
   ...
   * @reject {Error} - The error object.
   */
  beginPurgeWithHttpOperationResponse(resourceGroupName, resourceName, body, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._beginPurge(resourceGroupName, resourceName, body, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  beginPurge(resourceGroupName, resourceName, body, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._beginPurge(resourceGroupName, resourceName, body, options, optionalCallback);
    }
  }
