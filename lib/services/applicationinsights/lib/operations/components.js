 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _purgeStatus(resourceGroupName, resourceName, purgeId, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class Components {
  ...
  constructor(client) {
    ...
    this._purge = _purge;
    this._purgeStatus = _purgeStatus;
   ...
   * @reject {Error} - The error object.
   */
  purgeStatusWithHttpOperationResponse(resourceGroupName, resourceName, purgeId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._purgeStatus(resourceGroupName, resourceName, purgeId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  purgeStatus(resourceGroupName, resourceName, purgeId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._purgeStatus(resourceGroupName, resourceName, purgeId, options, optionalCallback);
    }
  }
