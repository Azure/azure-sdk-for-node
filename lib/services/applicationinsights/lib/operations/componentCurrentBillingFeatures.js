    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['ApplicationInsightsComponentBillingFeatures']().mapper();
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _update(resourceGroupName, resourceName, billingFeaturesProperties, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    ...
    if (billingFeaturesProperties === null || billingFeaturesProperties === undefined) {
      throw new Error('billingFeaturesProperties cannot be null or undefined.');
    }
    if (this.client.acceptLanguage !== null && this.client.acceptLanguage !== undefined && typeof this.client.acceptLanguage.valueOf() !== 'string') {
      throw new Error('this.client.acceptLanguage must be of type string.');
    }
  ...
  try {
    if (billingFeaturesProperties !== null && billingFeaturesProperties !== undefined) {
      let requestModelMapper = new client.models['ApplicationInsightsComponentBillingFeatures']().mapper();
      requestModel = client.serialize(requestModelMapper, billingFeaturesProperties, 'billingFeaturesProperties');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(billingFeaturesProperties, null, 2)}.`);
...
class ComponentCurrentBillingFeatures {
   ...
   * @reject {Error} - The error object.
   */
  updateWithHttpOperationResponse(resourceGroupName, resourceName, billingFeaturesProperties, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._update(resourceGroupName, resourceName, billingFeaturesProperties, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  update(resourceGroupName, resourceName, billingFeaturesProperties, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._update(resourceGroupName, resourceName, billingFeaturesProperties, options, optionalCallback);
    }
  }

}

module.exports = ComponentCurrentBillingFeatures;
