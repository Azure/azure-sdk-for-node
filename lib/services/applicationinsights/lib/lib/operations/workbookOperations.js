 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _list(sourceId, category, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
/** Class representing a WorkbookOperations. */
class WorkbookOperations {
  /**
   * Create a WorkbookOperations.
   ...
   * @reject {Error} - The error object.
   */
  listWithHttpOperationResponse(sourceId, category, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._list(sourceId, category, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  list(sourceId, category, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._list(sourceId, category, options, optionalCallback);
    }
  }

}

module.exports = WorkbookOperations;
