...
TableService.NSMETA = 'm';
TableService.NSDATA = 'd';
...
function TableService(storageAccount, storageAccessKey, host, authenticationProvider) {
  ...
  this.atomHandler = new AtomHandler(TableService.NSMETA, TableService.NSDATA);
}
...
TableService.prototype.getTable = function (table, optionsOrCallback, callback) {
  ...
  var processResponseCallback = function (responseObject, next) {
    ...
    if (!responseObject.error) {
      responseObject.tableResponse = self.atomHandler.parse(responseObject.response.body);
    }
...
TableService.prototype.createTable = function (table, optionsOrCallback, callback) {
  ...
  var xmlTableDescriptor = this.atomHandler.serialize({ TableName: table });

  var webResource = WebResource.post('Tables');
  webResource.addOptionalHeader(HeaderConstants.CONTENT_TYPE, 'application/atom+xml');
  webResource.addOptionalHeader(HeaderConstants.CONTENT_LENGTH, xmlTableDescriptor.length);
  ...
  var processResponseCallback = function (responseObject, next) {
    ...
    if (!responseObject.error) {
      ...
      tables.forEach(function (currentTable) {
        var parsedCurrentTable = self.atomHandler.parse(currentTable);
  ...
  var processResponseCallback = function (responseObject, next) {
    ...
    if (!responseObject.error) {
      ...
      entries.forEach(function (currentEntry) {
        var tableEntry = self.atomHandler.parse(currentEntry);
...
TableService.prototype.insertEntity = function (tableName, entityDescriptor, optionsOrCallback, callback) {
  ...
  var entityXmlDescriptor = this.atomHandler.serialize(entityDescriptor);
  ...
  if (this.isInBatch()) {
    ...
    var processResponseCallback = function (responseObject, next) {
      ...
      if (!responseObject.error) {
        responseObject.insertResponse = self.atomHandler.parse(responseObject.response.body);
