...
function _updateFace(personGroupId, personId, persistedFaceId, options, callback) {
  ...
  let body;
  if (userData !== null && userData !== undefined) {
    body = new client.models['UpdatePersonFaceRequest']();
    body.userData = userData;
  }
  ...
  try {
    if (body !== null && body !== undefined) {
      let requestModelMapper = new client.models['UpdatePersonFaceRequest']().mapper();
      requestModel = client.serialize(requestModelMapper, body, 'body');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _addPersonFaceFromUrl(personGroupId, personId, url, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class PersonGroupPerson {
   ...
   * @reject {Error} - The error object.
   */
  addPersonFaceFromUrlWithHttpOperationResponse(personGroupId, personId, url, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._addPersonFaceFromUrl(personGroupId, personId, url, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  addPersonFaceFromUrl(personGroupId, personId, url, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._addPersonFaceFromUrl(personGroupId, personId, url, options, optionalCallback);
    }
  }
