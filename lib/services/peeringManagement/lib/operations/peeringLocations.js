 ...
 */
function _list(kind, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'subscriptions/{subscriptionId}/providers/Microsoft.Peering/peeringLocations';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['PeeringLocationListResult']().mapper();
...
class PeeringLocations {
   ...
   */
  listWithHttpOperationResponse(kind, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._list(kind, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  list(kind, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._list(kind, options, optionalCallback);
    }
  }

}

module.exports = PeeringLocations;
