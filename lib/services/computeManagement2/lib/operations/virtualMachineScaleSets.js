 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getOSUpgradeHistory(resourceGroupName, vmScaleSetName, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Compute/virtualMachineScaleSets/{vmScaleSetName}/osUpgradeHistory';
  requestUrl = requestUrl.replace('{resourceGroupName}', encodeURIComponent(resourceGroupName));
  requestUrl = requestUrl.replace('{vmScaleSetName}', encodeURIComponent(vmScaleSetName));
  requestUrl = requestUrl.replace('{subscriptionId}', encodeURIComponent(this.client.subscriptionId));
  let queryParameters = [];
  ...
  return client.pipeline(httpRequest, (err, response, responseBody) => {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['VirtualMachineScaleSetListOSUpgradeHistory']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        let deserializationError = new Error(`Error ${error} occurred in deserializing the responseBody - ${responseBody}`);
 ...
 *
 * @param {array} [options.vmScaleSetReimageInput.dataDisks] Luns of dataDisks
 * in the virtual machine scale set VM to reimage.
 *
 * @param {object} [options.customHeaders] Headers that will be added to the
 * request
 *
...
function _beginReimage(resourceGroupName, vmScaleSetName, options, callback) {
  ...
  let vmScaleSetReimageInput = (options && options.vmScaleSetReimageInput !== undefined) ? options.vmScaleSetReimageInput : undefined;
  let apiVersion = '2018-10-01';
  // Validate
  try {
    if (resourceGroupName === null || resourceGroupName === undefined || typeof resourceGroupName.valueOf() !== 'string') {
  ...
  let requestModel = null;
  try {
    if (vmScaleSetReimageInput !== null && vmScaleSetReimageInput !== undefined) {
      let requestModelMapper = new client.models['VirtualMachineScaleSetReimageParameters']().mapper();
      requestModel = client.serialize(requestModelMapper, vmScaleSetReimageInput, 'vmScaleSetReimageInput');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    let serializationError = new Error(`Error "${error.message}" occurred in serializing the ` +
        `payload - ${JSON.stringify(vmScaleSetReimageInput, null, 2)}.`);
    return callback(serializationError);
  }
  httpRequest.body = requestContent;
  // Send Request
...
class VirtualMachineScaleSets {
  ...
  constructor(client) {
    ...
    this._getOSUpgradeHistory = _getOSUpgradeHistory;
    ...
    this._getOSUpgradeHistoryNext = _getOSUpgradeHistoryNext;
  }
   ...
   * @reject {Error} - The error object.
   */
  getOSUpgradeHistoryWithHttpOperationResponse(resourceGroupName, vmScaleSetName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getOSUpgradeHistory(resourceGroupName, vmScaleSetName, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getOSUpgradeHistory(resourceGroupName, vmScaleSetName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._getOSUpgradeHistory(resourceGroupName, vmScaleSetName, options, optionalCallback);
    }
  }
   ...
   * @reject {Error} - The error object.
   */
  getOSUpgradeHistoryNextWithHttpOperationResponse(nextPageLink, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getOSUpgradeHistoryNext(nextPageLink, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getOSUpgradeHistoryNext(nextPageLink, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._getOSUpgradeHistoryNext(nextPageLink, options, optionalCallback);
    }
  }
