/*
 * Copyright (c) Microsoft Corporation. All rights reserved.
 * Licensed under the MIT License. See License.txt in the project root for
 * license information.
 *
 * Code generated by Microsoft (R) AutoRest Code Generator.
 * Changes may cause incorrect behavior and will be lost if the code is
 * regenerated.
 */

'use strict';

/**
 * Model class for provider specific details for an event.
 *
 */
class EventProviderSpecificDetails {
  /**
   * Create a EventProviderSpecificDetails.
   * @member {string} instanceType Polymorphic Discriminator
   */
  constructor() {
  }

  /**
   * Defines the metadata of EventProviderSpecificDetails
   *
   * @returns {object} metadata of EventProviderSpecificDetails
   *
   */
  mapper() {
    return {
      required: false,
      serializedName: 'EventProviderSpecificDetails',
      type: {
        name: 'Composite',
        polymorphicDiscriminator: {
          serializedName: 'instanceType',
          clientName: 'instanceType'
        },
        uberParent: 'EventProviderSpecificDetails',
        className: 'EventProviderSpecificDetails',
        modelProperties: {
          instanceType: {
            required: true,
            serializedName: 'instanceType',
            type: {
              name: 'String'
            }
          }
        }
      }
    };
  }
}

module.exports = EventProviderSpecificDetails;
