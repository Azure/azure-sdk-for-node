...
* @param {object} rules        The rules to be applied.
...
*/
RoleParser.prototype.parse = function (roleSchema, role, rules, serializeTo, output) {
  for(var key in roleSchema) {
    ...
    if(typeInfo.isCollection) {
      this.handleCollectionType(value, role, key, rules, serializeTo, typeInfo, output);
    } else if(typeInfo.isComplex) {
      this.handleComplexType(value, role, key, rules, serializeTo, output);
    } else if (typeInfo.isPrimitive) {
      this.handlePrimitiveType(value, role, key, rules, serializeTo, output);
    }
  }
};
...
*/
RoleParser.prototype.handlePrimitiveType = function(metadata, role, key, rules, serializeTo, output) {
  var hasRule = (typeof metadata.Required !== 'boolean');
  if (hasRule) {
      if(!rules || !rules[metadata.Required]) {
        throw new Error('A rule ' + metadata.Required + ' is defined for the primitive type \'' +
          key + '\', but rules param is empty or not contain the definition of rule');
      }

      if(!(key in rules[metadata.Required]['flags'])) {
        throw new Error('A rule ' + metadata.Required + ' is defined for the primitive type \'' +
            key + '\', but flag for that does not exists');
      }
  }
  ...
  if(role[key]) {
    ...
    if(hasRule) {
      rules[metadata.Required]['flags'][key] = true;
    }
  } else if (hasRule) {
    rules[metadata.Required]['flags'][key] = false;
...
*                              type.
* @param {object} rules        The rules to be applied, we don't support rule for
*                              complex type, so will ignore this parameter
...
*                              associated meta-data.
*/
RoleParser.prototype.handleComplexType = function(metadata, role, key, rules2, serializeTo, output) {
  if(key in role) {
    var rules = null;
    if(metadata['@rules']) {
      // Rules are defined for the elements of the complex type
      // make a clone of this [Currently rules is supported for
      // for primitive member of complex type]
      rules = JSON.parse(JSON.stringify(metadata.TypeMeta['@rules']));
    }

    if(serializeTo === 'XML') {
      var newNode = output.ele(metadata.XmlNode);
      this.parse(metadata.TypeMeta, role[key], rules, serializeTo, newNode);
      newNode.up();
    } else {
      // JSON
    }
    ...
    if(rules) {
      for(var rule in rules) {
        if (eval(rules[rule]['expression'])) {
          throw new Error(rules[rule]['message']);
        }
      }
    }
  } else if(metadata.Required) {
...
*                              type.
* @param {object} rules        The rules to be applied, we don't support rule for
*                              collection type [only to primitive members of complex type]
*                              so will ignore this parameter
...
*                              associated meta-data.
*/
RoleParser.prototype.handleCollectionType = function(metadata, role, key, rules2, serializeTo, typeInfo, output) {
  if(key in role) {
    if(serializeTo === 'XML') {
      ...
      if(typeInfo.isComplex) {
        if(typeInfo.subType === 'Mixed') {
            ...
            for(var k = 0; k < role[key].length; k++) {
              if(keyProperty in role[key][k]) {
                if(role[key][k][keyProperty] === keyValues[j]) {
                  ...
                  this.parse(metadata.TypeMeta[keyValues[j]], role[key][k], null, serializeTo, newNodeLevel2);
                  newNodeLevel2.up();
                }
              } else {
          ...
          for(var i = 0; i < role[key].length; i++) {
            ...
            var newNodeLevel2 = newNodeLevel1.ele(metadata.ItemXmlNode);
            var rules = null;
            if(metadata.TypeMeta['@rules']) {
              // Rules are defined for the elements of this complex type
              // make a clone of this [Currently rules is supported for
              // for primitive member of complex type]
              rules = JSON.parse(JSON.stringify(metadata.TypeMeta['@rules']));
            }

            this.parse(metadata.TypeMeta, role[key][i], rules, serializeTo, newNodeLevel2);
            if(rules) {
              for(var rule in rules) {
                if (eval(rules[rule]['expression'])) {
                  throw new Error(rules[rule]['message']);
