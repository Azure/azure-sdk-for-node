  ...
  if (transform instanceof Array) {
    ...
    for (var key in transform) {
      if (transform.hasOwnProperty(key)) {
        // CLI expect object[key] as a required list. The 1st, 2nd and 3rd check make
...
ServiceManagementService.prototype.getNetworkConfig = function (callback) {
  validateObjectArgument(callback, 'callback', 'getVNetConfig');
...
ServiceManagementService.prototype.setNetworkConfig = function (networkConfiguration, callback) {
  ...
  var webResource = WebResource.put(path);
  var outbody = this.serialize.buildNetworkConfiguration(networkConfiguration, this);
