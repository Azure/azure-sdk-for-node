  ...
  this.performRequest(webResource, null, null, function (responseObject, next) {
    if (!responseObject.error) {
      ...
      if (responseObject.response.body.Servers && responseObject.response.body.Servers.Server) {
        var servers = responseObject.response.body.Servers.Server;
        servers.forEach(function (server) {
          responseObject.sqlServers.push({
            Name: server.Name[0],
            AdministratorLogin: server.AdministratorLogin[0],
...
SqlDatabaseService.prototype.createServer = function (administratorLogin, administratorLoginPassword, location, callback) {
  ...
  var serverBodyXml = js2xml.serialize(serverBody);

  this.performRequest(webResource, serverBodyXml, null, function (responseObject, next) {
    if (!responseObject.error) {
  ...
  this.performRequest(webResource, null, null, function (responseObject, next) {
    if (!responseObject.error) {
      ...
      if (responseObject.response.body.ServiceResources && responseObject.response.body.ServiceResources.ServiceResource) {
        var firewallRules = responseObject.response.body.ServiceResources.ServiceResource;
        firewallRules.forEach(function (firewallRule) {
          responseObject.firewallRules.push({
            Name: firewallRule.Name[0],
            StartIPAddress: firewallRule.StartIPAddress[0],
            EndIPAddress: firewallRule.EndIPAddress[0]
          })
        });
      }
    }
...
SqlDatabaseService.prototype.createServerFirewallRule = function (serverName, ruleName, startIPAddress, endIPAddress, callback) {
  ...
  var ruleBodyXml = js2xml.serialize(ruleBody);

  this.performRequest(webResource, ruleBodyXml, null, function (responseObject, next) {
    if (!responseObject.error) {
      responseObject.firewallRule = [];

      if (responseObject.response.body.ServiceResource) {
        responseObject.firewallRule = {
          Name: responseObject.response.body.ServiceResource.Name[0],
          StartIPAddress: responseObject.response.body.ServiceResource.StartIPAddress[0],
          EndIPAddress: responseObject.response.body.ServiceResource.EndIPAddress[0]
        };
      }
    }
