...
var RoleAssignmentOperations = ( /** @lends RoleAssignmentOperations */ function() {
   ...
   * id.
   * 
   * @param {string} [parameters.principalId] Gets or sets principal Id.
   * 
   ...
   * @param {string} scope Scope.
   * 
   * @param {string} [principalId] User principal id. if null role assignments
   ...
   * @returns {Stream} The response stream.
   */
  RoleAssignmentOperations.prototype.listForScope = function(scope, principalId, callback) {
    if (callback === null || callback === undefined) {
      throw new Error('callback cannot be null.');
    }
    // Validate
    ...
    if (principalId) {
      url2 = url2 + '&filter=principalId eq ' + encodeURIComponent(principalId ? principalId.trim() : '');
    }
    url2 = url2 + '&api-version=2014-07-01-preview';
    var baseUrl = this.client.baseUri;
    // Trim '/' character from the end of baseUrl and beginning of url.
