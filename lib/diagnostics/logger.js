...
var zeroPaddingString = azureutil.zeroPaddingString;
...
Logger.prototype.defaultLoggerFunction = function(logLevel , msg) {
  ...
  var time = new Date();
  //var timeStamp = zeroPaddingString(time.getHours(), 2) + ':' + zeroPaddingString(time.getMinutes(), 2) +
  //  ':' + zeroPaddingString(time.getSeconds(), 2) + ':' + zeroPaddingString(time.getMilliseconds(), 3);
  var hrtime = process.hrtime();
  var p = 1000000;
  var ntime = hrtime[1] % p;
  var mtime = parseInt(hrtime[1] / p);
  var timeStamp = zeroPaddingString(hrtime[0] % 1000, 3) + ':' + zeroPaddingString(mtime, 3) + ":" + zeroPaddingString(ntime, 6);
