...
_.extend(exports, {
  ...
  addAttribute: function (element, attribute) {
    element.attr[attribute.name] = attribute;
  },
  ...
  serializeDocument: function (element) {
    ...
    var rootElement = generatedDoc.begin(self._getName(namespaces, element.name, element.namespace, true), { version: '1.0', encoding: 'utf-8', standalone: 'yes' });
  ...
  _getName: function (namespaces, name, namespace, isRoot) {
    var self = this;

    if (namespace) {
      if (namespaces[namespace] === undefined) {
        if (isRoot && Object.keys(namespaces).length === 0) {
          namespaces[namespace] = null;
        } else {
          var prefixIndex = 96 + Object.keys(namespaces).length;
          if (!self._hasRoot(namespaces)) {
            prefixIndex++;
          }

          // Letters starting with 'a' = 97
          namespaces[namespace] = String.fromCharCode(prefixIndex);
        }
      }
  ...
  _writeElement: function (xmlElement, element, namespaces) {
    ...
    if (element.attr) {
      Object.keys(element.attr).forEach(function (attrName) {
        ...
        var attributeName = self._getName (namespaces, attribute.name, attribute.namespace);
        xmlElement = xmlElement.att(attributeName, element.attr[attribute.name].value);
      });
    }
