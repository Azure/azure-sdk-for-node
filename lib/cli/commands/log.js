...
exports.init = function(cli) {
  ...
  diagnostic.command("tail [name]").whiteListPowershell().description("Download diagnostic log").option("-s, --subscription <id>", "use the subscription id").option("-p, --path <path>", "log path under LogFiles folder").execute(function __2(name, options, _) { var context, repositoryUri, buf; var __frame = { name: "__2", line: 79 }; return __func(_, this, arguments, __2, 2, __frame, function __$__2() {
      ...
      return site.ensureRepositoryUri(context, __cb(_, __frame, 9, 26, function ___(__0, __1) { repositoryUri = __1; return (function __$__2(__then) {
          if (repositoryUri) {
            return doLogStream(context, __cb(_, __frame, 11, 18, function ___(__0, __2) { buf = __2;



              log.info(buf); __then(); }, true), function(err, line) { process.stdout.write(line); }); } else {

            log.error("Repository is not setup"); __then(); } ; })(_); }, true)); }); });



  function doDownloadDiagnostic(context, _) { var channel, progress; var __frame = { name: "doDownloadDiagnostic", line: 100 }; return __func(_, this, arguments, doDownloadDiagnostic, 1, __frame, function __$doDownloadDiagnostic() {
  ...
  function doLogStream(context, _, chunkcb) { var channel; var __frame = { name: "doLogStream", line: 111 }; return __func(_, this, arguments, doLogStream, 1, __frame, function __$doLogStream() {

      channel = scm.getScmChannel(context).path("logstream");
      if (context.path) {
        channel = channel.path(context.path); } ;


      return channel.GET(__cb(_, __frame, 7, 11, _, true), chunkcb); }); };};