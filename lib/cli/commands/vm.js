...
exports.init = function(cli) {
  ...
  vm.command('create <dns-name> <image> <user-name> [password]')
    ...
    .option('-t, --ssh [port]', 'enable SSH [22]')
      ...
      if (options.rsps.length === 0 && options.errs.length > 0) {
        ...
        if (found) {
          if (options.attach) {
            ...
            utils.doServiceManagementOperation(channel, 'removeDataDisk', found.svc, found.deploy.Name, options.name, parseInt(options.lun, 10), function(error, response) {
              progress.end();
              cmdCallback(error);
            });
          }
