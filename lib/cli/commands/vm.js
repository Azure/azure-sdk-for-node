...
exports.init = function(cli) {
  ...
  endpoint.command('create <vm-name> <lb-port> [vm-port]')
    ...
    .option('-t, --prob-port <port>', 'VM port to use to inspect the role availability status')
    .option('-r, --prob-protocol <protocol>', 'protocol to use to inspect the role availability status')
    .option('-p, --prob-path <path>', 'relative path to inspect the role availability status')
    .execute(function(vmName, lbport, vmport, options, callback) {
      ...
      var probportAsInt = -1;
      var prob = options.probPort !== undefined || options.probProtocol !== undefined || options.probPath !== undefined;
      if (prob) {
        if (!options.lbSetName) {
          callback('The optional argument --lb-set-name is required to enable probing');
        }

        if (!options.probPort || !options.probProtocol) {
          callback('The optional arguments --prob-port and --prob-protocol are required to enable probing');
        }

        probportAsInt = parseInt(options.probPort, 10);
        if ((options.probPort != probportAsInt) || (probportAsInt > 65535)) {
          callback('--prob-port must be an integer less than or equal to 65535');
        }

        if (options.probProtocol === 'tcp') {
          if (options.probPath) {
            options.probPath = undefined;
            logger.warn('--prob-path option will be ignored when --prob-protocol is tcp');
          }
        } else if (options.probProtocol === 'http') {
          if (!options.probPath) {
            options.probPath = '/';
          }
        } else {
          callback('Possible values for --prob-protocol are tcp and http');
        }
      }
      ...
      endpointCreateDelete({
        ...
        probport: probportAsInt,
        probprotocol: options.probProtocol,
        probpath: options.probPath,
        create: true
      }, callback);
    });
              ...
              if (m !== endpointCount) {
                ...
                if (options.create) {
                  ...
                  if (options.lbsetname) {
                    ...
                    if (options.probport !== -1) {
                      inputEndPoint["LoadBalancerProbe"] = {
                          Port: options.probport,
                          Protocol: options.probprotocol
                      };

                      if (options.probpath) {
                        inputEndPoint["LoadBalancerProbe"]["Path"] = options.probpath;
                      }
                    }
                  }
