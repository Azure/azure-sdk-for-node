...
exports.init = function(cli) {
  ...
  var svcopts = {
    Label: '',
    Description: 'Implicitly created hosted service' + utils.getUTCTimeStamp()
  };
  ...
  function deleteAppIfEmptyAndImplicit(channel, dnsPrefix, callback) {
    utils.doServiceManagementOperation(channel, 'getHostedService', dnsPrefix, function(error, response) {
      if (!error) {
        if (response.body.HostedServiceProperties.Description &&
            (response.body.HostedServiceProperties.Description.indexOf('Implicitly created hosted service') === 0)) {
          var options = {
            dnsPrefix: dnsPrefix,
            useprod: true,
            usestage: true
