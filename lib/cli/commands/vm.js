...
exports.init = function(cli) {
  ...
  vm.command('create <dns-prefix> <image> <user-name> [password]')
    ...
    .option('-s, --ssh [port]', 'enable SSH [22]')
    .option('-t, --ssh-cert [pem-file|fingerprint]', 'SSH certificate')
    .option('-r, --rdp [port]', 'enable RDP [3389]')
    .option('-i, --subscription <id>', 'subscription id')
    .execute(function(dnsPrefix, image, userName, password, options, callback) {    
      var vmSize;
      if (options.vmSize) {
      ...
      // Verify that dnsPrefix is valid
      if (dnsPrefix.indexOf('.') !== -1) {
        callback('Invalid dns-prefix');
      }
  ...
  vm.command('create-from <dns-prefix> <role-file>')
    ...
    .option('-l, --location <name>', 'location of the data center')
    .option('-a, --affinity-group <name>', 'affinity group')
    .option('-s, --ssh [cert]', 'upload SSH and upload ssh cert to VM') // $TODO: make ssh work with create-from
    .option('-i, --subscription <id>', 'use the subscription id')
    .execute(function(dnsPrefix, roleFile, options, callback) {
      logger.verbose('Loading role file: ' + options.roleFile);
      var role = require(options.roleFile);
  ...
  endpoint.command('create <vm-name> <lb-port> [vm-port]')
    ...
    .option('-i, --subscription <id>', 'use the subscription id')
    .option('-d, --dns-prefix <name>', 'only show VMs for this DNS prefix')
    .execute(function(name, lbport, vmport, options, callback) {
  ...
  endpoint.command('delete <vm-name> <lb-port>')
    ...
    .option('-i, --subscription <id>', 'use the subscription id')
    .option('-d, --dns-prefix <name>', 'only show VMs for this DNS prefix')
    .execute(function(name, lbport, options, callback) {
  ...
  datadisk.command('attach-new <vm-name> <size-in-gb> [blob-url]')
    ...
    .execute(function(name, size, blobUrl, options, callback) {
      ...
      var sizeAsInt = parseInt(size, 10);
      if (size != sizeAsInt) {
        ...
        if (error) {
          ...
          if (service) {
            ...
            if (sshFingerprint) {
              ...
              configureCert(service.ServiceName, function(error, response) {
                progress.end();
                if (!error) {
            ...
            svcopts.Location = options.location;
        ...
        if (found) {
          ...
          deleteRoleOrDeployment(channel, found.svc, found.deploy, name, function(error) {
            ...
            if (!error) {
              ...
              progress = cli.progress('Deleting blob' + (dataDiskCount ? 's' : ''));
              var doneCount = 0;
              var errorCount = 0;
              var allCount = dataDiskCount + 1;
              for (var k = -1; k < dataDiskCount; ++k) {
          ...
          utils.doServiceManagementOperation(channel, 'getRole', found.svc, found.deploy.Name, options.name, function(error, response) {
            if (!error) {
              ...
              for (; m < endpointCount; m++) {
                if (configurationSets[k].InputEndpoints[m].Port === options.lbport) {
                ...
                if (configurationSets[k].InputEndpoints[m].LocalPort === options.vmport) {
              ...
              if (m !== endpointCount) {
                ...
                if (options.create) {
                  var inputEndPoint = {
                    Name: 'endpname-' + options.lbport + '-' + options.vmport,
                    Protocol: 'tcp',
                    Port: options.lbport,
        ...
        if (found) {
          if (options.attach) {
            ...
            for (var k = 0; k < found.dataVirtualHardDisks.length; k++) {
              luns.push(found.dataVirtualHardDisks[k].Lun);
            }

            luns.sort();
            var nextLun = 2;
            nextLun = luns.length > 0 ? luns[luns.length - 1] + 1 : nextLun;
  ...
  function listDataDisks(options, cmdCallback) {
    var channel = utils.createServiceManagementService(cli.category('account').lookupSubscriptionId(options.subscription),
      cli.category('account'), logger);
    ...
    enumDeployments(channel, options, function() {
      ...
      if (options.rsps.length === 0 && options.errs.length > 0) {
        ...
        // got unique role under a deployment and service, list the endpoints
        if (found) {
          logger.table(found.dataVirtualHardDisks, function (row, item) {
  ...
  function enumDeployments(channel, options, callback) {
    // get deployment by slot. Checks which slots to query.
    var getDeploymentSlot = function() {
      var dnsPrefix = options.dnsPrefix;
      options.pending++;
      utils.doServiceManagementOperation(channel, 'getDeploymentBySlot', options.dnsPrefix, 'Production', function(error, response) {
        options.pending--;
        if (error) {
