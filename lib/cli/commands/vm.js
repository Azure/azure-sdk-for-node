...
exports.init = function(cli) {
  ...
  vm.command('create <dns-name> <image> <user-name> [password]')
    ...
    .option('-w, --virtual-network-name', 'virtual network name')
    .option('-b, --subnet-names', 'comma-delimited subnet names')    
    ...
    .execute(function(sourcePath, blobUrl, storageAccountKey, options, callback) {
    	if (/^https?\:\/\//i.test(sourcePath)) {
    	  ...
    	  pageBlob.copyBlob(sourcePath, blobUrl, storageAccountKey, function(error, blob, response) {
    	    ...
    	    if (!error) {
    	      logger.silly('Status : ' + response.headers['x-ms-copy-status'] || response.headers['x-ms-copy-blob-status']);
    	    }
    	    callback(error);
    	  });
    	} else {
      ...
      function createDefaultRoleWithPassword_() {
        ...
        if (inputEndPoints.length !== 0) {
          role.ConfigurationSets.push({
            ...
            SubnetNames: options.subnetNames ? options.subnetNames.split(',') : undefined
          });
        }
