...
exports.init = function(cli) {
  ...
  osImage.command('create <name> [source-path]')
    ...
    .option('-k, --source-key <key>', 'Storage account key if source-path is uri and is private')
  ...
  disk.command('upload <source-path> <blob-url> <storage-account-key>')
    ...
    .execute(function(sourcePath, blobUrl, storageAccountKey, options, callback) {
    	if (/^https?\:\/\//i.test(sourcePath)) {
         ...
         pageBlob.copyBlob(sourcePath, null, blobUrl, storageAccountKey, function(error, blob, response) {
    	    progress.end();
