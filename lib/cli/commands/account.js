...
exports.init = function (cli) {
  ...
  account.command('import <file>')
        ...
        .execute(function (file, options, callback) {
          ...
          cacheUtils.clear();
          if (publishSettings) {
            return cli.category('site').doSpacesGet({ subscription: publishSettings.PublishProfile.Subscription['@'].Id }, callback);
          }
          return callback();
  ...
  // TODO, suwatch: remove after test migrating to 'azure vm location list'
  var location = account.category('location')
  ...
  location.command('list')
        ...
        .execute(function (options, callback) {
          listLAG('Locations', options, callback);
        });
