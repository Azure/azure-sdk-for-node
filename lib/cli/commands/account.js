...
exports.init = function (cli) {
  ...
  account.command('set')
    ...
    .execute(function (name, options, callback) {
      ...
      var filtered = subscriptions.filter(function (subscription) { return subscription.Name === name });
      if (!filtered.length) {
        // If nothing was found try matching on Id
        importByName = false;
        filtered = subscriptions.filter(function (subscription) { return subscription.Id === name });
      ...
      var subscription = filtered[0];
      var cfg = account.readConfig();

      var subscriptionTag = importByName ? subscription.Name : subscription.Id;
      log.info('Setting subscription to \'' + subscriptionTag + '\'');
      cfg['Subscription'] = subscription.Id;
