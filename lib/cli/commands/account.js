...
exports.init = function (cli) {
    ...
    account.command('clear')
        ...
        .action(function () {
            log.info('Clearing account info. (Not yet implemented.)');
        });
      ...
      utils.doServiceManagementOperation(channel, 'list' + what, function(error, response) {
        ...
        if (!error) {
          log.table(response.body, function (row, item) {
            ...
            if (item.DisplayName) { // for location
        ...
        if (callback) {
          callback(error);
        } else if (error) { // TODO make sure callback is passed
          cli.exit('error', error, 1);
        }
      });
    }
