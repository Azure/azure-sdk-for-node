...
exports.init = function (cli) {
  ...
  site.command('create [name]')
        ...
        .execute(function (nameArg, options, _) {
          ...
          function initGitOnCurrentDirectory(_) {
            ...
            if (context.publishingUsers['@'] && context.publishingUsers['@']['i:nil'] === 'true') {
          ...
          function addRemoteToLocalGitRepo(_) {
            ...
            var publishingUser = toArray(context.publishingUsers)[0];
            ...
            var repoUrl = url.parse(context.repo);

            repoUrl.protocol = 'http:';
            repoUrl.auth = publishingUser;

            var sitePath = context.site.name + '.git';
  ...
  function portalGitInitInstruction(context, _) {
    ...
    log.help('2. Click on the site named "' + ((context.site && context.site.name) || '{site name}') + '"');
  ...
  site.command('show [name]')
        ...
        .execute(function (name, options, _) {
          ...
          log.data('GitRepositoryUri', repositoryUri ? (repositoryUri + '/' + context.site.name + '.git') : 'none');
        });
