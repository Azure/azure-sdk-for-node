...
exports.init = function (cli) {
    ...
    site.command('create [name]')
        ...
        .execute(function (nameArg, options, _) {
            ...
            if (!options.git) {
                try {
                    log.silly('Detecting if local folder is already a git repo');
                    // assume --git options enabled when you're in a local git working tree
                    var isInsideWorkTree = exec('git rev-parse --is-inside-work-tree', _);
                    var lines = isInsideWorkTree.stdout + isInsideWorkTree.stderr;
                    options.git = lines.split('\n').some(function (line) {
                        return line === 'true';
                    });
                    if (options.git) {
                        log.verbose('Local git working tree detected');
                    }
                }
                catch (err) {
                    // error invoking git
            ...
            if (options.git) {
                ...
                var repoUrl = url.parse(repo + context.site.name + '.git');
                repoUrl.protocol = 'http:';
                repoUrl.auth = toArray(publishingUsers)[0];
                ...
                try {
                    ...
                    log.warn('Trouble updating local git folder', err);
                }
        ...
        .execute(function (name, options, _) {
            ...
            var result = getChannel()
                ...
                .PUT(function (req) {
                    ...
                    req.write(context.site.name + '.antdir0.antares-test.windows-int.net');
                    req.write('</string>');
                    req.write('</HostNames>');
                    req.write('<Name>');
