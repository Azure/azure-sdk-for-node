...
exports.init = function (cli) {
    ...
    site.command('portal [name]')
        ...
        .execute(function (name, options, _) {
            ...
            var url = 'http://commonuxfx-bvt01.cloudapp.net/?wa=wsignin1.0';
            if (options.name) {
                url = url + '#Workspaces/WebsiteExtension/Website/' + name + '/dashboard';
            }
    ...
    site.command('browse <name>')
        ...
        .execute(function (name, options, _) {
            ...
            log.json(siteData);
            var url = 'http://' + toArray(siteData.HostNames)[0];           
            log.verbose('Browsing url', url); 
            child_process.spawn('open', [url]);
        });
