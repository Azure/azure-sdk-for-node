...
/*   215 */         authorLength = 0;
/*   215 */         messageLength = 0;
/*   216 */         if ((deployments && deployments.length)) {
/*   217 */           log.table(deployments, function(row, deployment) {
/*   218 */             row.cell("Time", deployment.start_time);
/*   219 */             row.cell("Commit id", deployment.shortId);
/*   220 */             row.cell("Status", deployment.status);
/*   221 */             authorLength = Math.max(deployment.author.length, authorLength);
/*   222 */             row.cell("Author", deployment.author, null, Math.min(authorLength, 15));
/*   223 */             messageLength = Math.max(deployment.message.length, messageLength);
/*   224 */             row.cell("Message", deployment.message, null, Math.min(messageLength, 40));
                      });
                    }
/*   226 */          else {
/*   227 */           log.info("No git deployment found");
                    }
                  ;
                    _();
                  }, true));
...
/*   231 */   function showLogDetails(context, _) {
...
/*   233 */       return scm.doLogGet(context, __cb(_, __frame, 2, 15, function ___(__0, __2) {
                    logs = __2;
                    return (function __$showLogDetails(__then) {
/*   234 */           if ((logs && logs.length)) {
/*   235 */             progress = cli.progress("Retrieving log details");
                        return (function ___(__then) {
                          (function ___(_) {
                            __tryCatch(_, function __$showLogDetails() {
/*   237 */                   return async.map(logs, function __1(log, _) {
                                var details;
                                var __frame = {
                                  name: "__1",
                                  line: 237
                                ...
                                return __func(_, this, arguments, __1, 1, __frame, function __$__1() {
                                  return (function __$__1(__then) {
/*   238 */                         if (log.hasDetails) {
/*   244 */                           return getScmChannel(context).path("deployments").path(context.id).path("log").path(log.id).GET(__cb(_, __frame, 7, 26, function ___(__0, __1) {
                                        details = __1;
/*   245 */                             return _(null, details.map(formatLog));
                                      }, true));
                                    }
                                     else {
                                      __then();
...
/*   249 */                     progress.end();
                          ...
                          __tryCatch(_, function __$showLogDetails() {
/*   251 */                 for (i = 0; (i < logs.length); ++i) {
/*   252 */                   displayLog(logs[i]);
/*   253 */                   if (results[i]) {
/*   254 */                     details = results[i];
/*   255 */                     for (j = 0; (j < details.length); ++j) {
/*   256 */                       displayLog(details[j]);
                                };
                              }
                            ;
                            };
...
/*   261 */             log.info("deployment has no detail");
                        __then();
                      }
                    ;
                    })(_);
...
/*   265 */   function displayLog(item) {
/*   266 */     if ((item.type === "Warning")) {
/*   267 */       log.warn(((item.short_time + " ") + item.message));
                }
/*   268 */      else if ((item.type === "Error")) {
/*   269 */       log.error(((item.short_time + " ") + item.message));
                }
/*   270 */      else {
/*   271 */       log.data(((item.short_time + " ") + item.message));
                }
                
              ;
              };
/*   275 */   function fromJsonDate(str) {
/*   276 */     return eval(str.replace(/\/Date\((.*)[+].*\)\//gi, "new Date($1)"));
              };
/*   279 */   function formatDate(dt) {
/*   280 */     var date = dt.getDate(), month = dt.getMonth();
/*   282 */     date = ((((date < 10) ? "0" : "")) + date);
/*   283 */     month = ((((month < 10) ? "0" : "")) + month);
/*   284 */     return ((((((dt.getFullYear() + "-") + month) + "-") + date) + " ") + dt.toLocaleTimeString());
              };
/*   287 */   function dateTimeText(str) {
/*   288 */     return formatDate(fromJsonDate(str));
              };
/*   291 */   function deploymentStatusText(status) {
/*   292 */     switch (status) {
/*   293 */     case 0:
/*   293 */       return "Pending";
/*   293 */     case 1:
...
/*   302 */   function ensureShortCommitId(deployments) {
/*   303 */     return deployments.map(function(deployment) {
/*   304 */       deployment.shortId = deployment.id.substr(0, 10);
/*   304 */       return deployment;
/*   304 */     });
              };
/*   305 */   function ensureRepositoryUri(context, _) {
/*   305 */     var siteData, repositoryUri;
/*   305 */     var __frame = {
                  name: "ensureRepositoryUri",
/*   306 */       line: 318
/*   306 */     };
/*   306 */     return __func(_, this, arguments, ensureRepositoryUri, 1, __frame, function __$ensureRepositoryUri() {
                  return site.lookupSiteNameAndWebSpace(context, __cb(_, __frame, 1, 19, function ___(__0, __1) {
                    siteData = __1;
/*   307 */         repositoryUri = (siteData && site.getRepositoryUri(siteData));
/*   307 */         return (function __$ensureRepositoryUri(__then) {
                      if (!repositoryUri) {
                        return site.doSiteGet(context, __cb(_, __frame, 4, 17, function ___(__0, __2) {
                          siteData = __2;
/*   311 */               repositoryUri = site.getRepositoryUri(siteData);
/*   312 */               __then();
/*   313 */             }, true));
/*   314 */           }
                       else {
...
/*   321 */     });
/*   322 */   };
/*   322 */   scm.ensureRepositoryUri = ensureRepositoryUri;
/*   323 */   function formatDeployment(deployment) {
...
/*   325 */     deployment.status = (deployment.active ? "Active" : deploymentStatusText(deployment.status));
/*   326 */     deployment.message = deployment.message.replace(/\s*(.*)\s*?/g, "$1");
/*   327 */     delete deployment.active;
...
/*   330 */     log.log_time = (log.log_time && dateTimeText(log.log_time));
/*   332 */     log.short_time = (log.log_time && log.log_time.replace(/.* +(.*)/g, "$1"));
/*   333 */     log.type = logTypeText(log.type);
/*   334 */     log.shortId = log.id.substr(0, 10);
/*   335 */     delete log.details_url;
/*   336 */     return log;
              };
            };
