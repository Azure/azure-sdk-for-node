...
function harvestModules() {
    ...
    function scan(scanPath) {
        ...
        results = results.filter(function(item) {
            item.package = JSON.parse(fs.readFileSync(item.packagePath));
            return item.package && item.package.plugins && item.package.plugins['azure-cli'];
        });

        results = flatten(results.map(function(item) {
            var plugins = item.package.plugins['azure-cli'];
