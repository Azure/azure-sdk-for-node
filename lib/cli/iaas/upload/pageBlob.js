...
var uploadPageBlobFromBlobService = 
  ...
  sillyFunc('Uploading page blob ' + fileName + 'to /' + blobResourceName);
  ...
  if (options.vhd) {
    ...
    if (vhdInfo.ddHeader) { // dynamic or difference
      dynVHD = true;
      size = vhdInfo.footer.currentSize + 512; // add footer
      verboseFunc('Dynamic or difference VHD: converting to fixed type. Fixed VHD size = ' + size / 1024 + ' KB');
  ...
  var md5Agg = callbackAggregator.callbackAndAggregator(function() {
    verboseFunc('Done!');
    callback(null);
  });
  ...
  function onGotBlobInfo(error, result) {
    if (error) {
      errorFunc('Cannot access existing blob /' + blobResourceName + 
  ...
  function onBlobCreated(error) {
    ...
    var alreadyDone = 0; // previously uploaded blocks - to adjust speed computation
    ...
    function printStatus() {
      ...
      if ((doneBlocks >= numThreads || percentCompleted > 90) && uploadedBlocks > 0) {
        ...
        var kbps = (blockSize / 1024 * doneBlocks) / elapsedTime;
      ...
      var runningRequests = runningThreads;
      var string = 
