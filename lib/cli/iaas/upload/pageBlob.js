...
var copyBlob =
  ...
  var splitSource = splitDestinationUri(sourceUri);
  if (!splitSource.accountName || !splitSource.blobName) {
    throw new Error('Source is not well formed :' + sourceUri);
  }
  
  blobService.copyBlob(splitSource.container || '$root', splitSource.blobName, splitDestination.container || '$root', splitDestination.blobName,
      {sourceAccount : splitSource.accountName}, checkStatus);
  ...
  function checkStatus(error, blob, response) {
    if (!error) {
      ...
      if (status.toLowerCase() !== 'success') {
        setTimeout(function() {
          blobService.copyBlobStatus(splitDestination.container || '$root', splitDestination.blobName, checkStatus);
        }, 3000);
        return;
      }
    }
