  ...
  return function(name, sourcePath, options, callback) {
    ...
    function upload() {
      if (sourcePath) { // uploading
        if (/^http\:\/\//i.test(sourcePath)) {
          logger.verbose('Copying blob from ' + sourcePath + ' to ' + genBlobUrl);
          if (options.md5Skip || options.parallel || options.baseVhd) {
            logger.warn('--md5-skip, --parallel and/or --base-vhd options will be ignored');
          }
          if (!options.forceOverwrite) {
            logger.warn('Any existing blob will be overwritten at ' + blobUrl);
          }
          uploadVMImage.copyBlobFromIaasClient(channel, sourcePath, genBlobUrl, 
              options2, function (error, blob, response, newDestUri) {
            logger.silly('Status code = ' + response.statusCode);
            if (!error) {
              var status = response.headers['x-ms-copy-blob-status'];
              (status === 'success' ? logger.verbose : logger.warn)('Copied blob, status : ' + status);
              // TODO poll if status is not 'success'
