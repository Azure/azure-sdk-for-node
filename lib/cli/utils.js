...
exports.validateEndpoint = function (endpoint) {
  ...
  var port = (parts.port && parseInt(parts.port, 10)) || (/https/i.test(parts.protocol) ? 443 : 80);
  return { endpointHost: parts.hostname, endpointPort: port };
};
...
function azureDir() {
  if (process.env.AZURE_CONFIG_DIR) {
    return process.env.AZURE_CONFIG_DIR;
  }
