                ...
                site.doRepositoryPost(parameters, function () {
                    site.doRepositoryGet(parameters, function (repo) {
                        site.doRepositoryGet(parameters, function (account) {
                            log.info('To start adding content to the website, type in the following:');
                            log.info('  git init');
                            log.info('  git add .');
                            log.info('  git commit -m "initial commit"');
                            log.info('  git remote add azure http://{UserName}@repository-{Name}.antintdublin.dnsremap.com/git');
                            log.info('  git push azure master');
                        });
                    });
                });
            });
