...
_.extend(exports, {
  stripNamespaces: function (parsedDoc) {
    var self = this;

    var doc = {};

    for (var key in parsedDoc) {
      if (_.isObject(parsedDoc[key]) && !_.isArray(parsedDoc[key])) {
        doc[key] = self.stripNamespaces(parsedDoc[key]);
      } else {
        var pos = key.indexOf(':');
        if (pos >= 0) {
          doc[key.substr(pos + 1)] = parsedDoc[key];
        } else {
          doc[key] = parsedDoc[key];
        }
      }
    }
