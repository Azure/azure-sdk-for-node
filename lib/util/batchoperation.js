...
var os = require('os');
var DEFAULT_OPERATION_MEMORY_USAGE = require('./constants').BlobConstants.DEFAULT_WRITE_BLOCK_SIZE_IN_BYTES;
...
function BatchOperation(name, options) {
  ...
  //Action operations count(The operations which are connecting to remote or executing callback)
  this._activeOperation = 0;
  //Queued operations count(The operations which are waiting for connecting to remote)
  this._queuedOperation = 0;
  //Connected operations count(The operations which are connecting to remote)
  this._connectedOperation = 0;
...
BatchOperation.prototype.IsWorkloadHeavy = function() {
  ...
  return (this._activeOperation + this._queuedOperation)>= 5 * globalConcurrency ||
    (this._activeOperation >= globalConcurrency && this._getApproximateMemoryUsage() > 0.5 * SystemTotalMemory);
};
...
BatchOperation.prototype._getApproximateMemoryUsage = function() {
  ...
  var futureUsage = (this._queuedOperation + this._connectedOperation) * this.operationMemoryUsage;
...
//      return;
//    }
////  } else if(!this._isFullConnection()){
//  } else {
//    var operation = null;
...
//    }
//
//    this.logger.debug(util.format('Start idle operation finished. Total:%d, Active:%d, Connected: %d, Queued:%d',
//      this._totalOperation, this._activeOperation, this._connectedOperation, this._queuedOperation));
//  }
//};

///**
//* Is the concurrent connection full
//*/
//BatchOperation.prototype._isFullConnection = function () {
//  return this._connectedOperation >= globalConcurrency;
//};
...
BatchOperation.prototype._runOperation = function (operation) {
  ...
  this._queuedOperation--;
  this._connectedOperation++;
};

/**
* Release one connection and immediately start another connection if workload is not heavy
*/
//BatchOperation.prototype._releaseConnection = function() {
//  this._connectedOperation--;
//  this._startIdleOperation();
//};
...
//      process.nextTick(function(){
        //Download/upload are I/O-bound instead of Compute-bound tasks.
        //We can start async operations and fire user callback at the same time
