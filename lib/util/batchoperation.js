...
//TODO change to a small value
var DEFAULT_GLOBAL_CONCURRENCY = 40; //Default http connection limitation for nodejs
...
function CommonOperation(operation, callback) {
  this.status = OperationState.Inited;
  this.operationId = -1;
  this._callbackArguments = null;
  var sliceStart = 2;
  ...
  this.run = function(cb) {
    ...
    operationArguments.push(cb);
    this.status = OperationState.RUNNING;
    operation.apply(null, operationArguments);
