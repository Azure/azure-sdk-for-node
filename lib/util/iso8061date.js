...
exports.format = function (date, skipMilliseconds, millisecondsPading) {
  ...
  if (!skipMilliseconds) {
    if (!millisecondsPading) {
      millisecondsPading = 7;
    }

    components.push('.');
    components.push(rightPad(leftPad(date.getUTCMilliseconds(), 3), millisecondsPading));
  }
...
exports.parse = function (stringDateTime) {
  ...
  // Make sure it is padded for 7 digits
  ms[0] = rightPad(ms[0], 7);
  ...
  var date = new Date(Date.UTC(
    ...
    parseInt(hms[1], 10),
    parseInt(hms[2], 10),
    Math.round(parseInt(ms[0], 10) / 10000)
  ));
