...
var exec = (cmd, args, opts) => {
  args || (args = {});
  opts || (opts = {});
  return new Promise((resolve, reject) => {
    var child = require('child_process').execFile(cmd, args, opts,
      (err, stdout, stderr) => {
        if (err) {
          err.out = stdout;
          err.err = stderr;
...
describe('ServerManagement', function () {
  ...
  before(function (done) {
    ...
    suite.Get = (variableName, defaultValueFn) => {
      if (suite.isPlayback) {
  ...
  it('can create and remove gateways', function () {
    gatewayName = suite.Get("createremovegw", () => rnd('sdk-test-gw-'));

    return client.gateway.create(resourceGroupName, gatewayName, { location: location }).should.be.fulfilled().then((gateway) => {
      ...
      return client.gateway.get(resourceGroupName, gatewayName).should.be.fulfilled().then((gateway) => {
        gateway.type.should.equal("microsoft.servermanagement/gateways");
        gateway.location.should.equal(location);
        gateway.name.should.equal(gatewayName);
      ...
      return client.gateway.deleteMethod(resourceGroupName, gatewayName).should.be.fulfilled().then(() => {
  ...
  it('should not find random gateway', function () {
    var gatewayName = suite.Get("randomgatewayfail", () => rnd('sdk-test-gw-'));
    return client.gateway.get(resourceGroupName, gatewayName).should.be.rejected();
  });

  it('list the gateways in the resourcegroup', function () {
    return client.gateway.list(resourceGroupName).should.be.fulfilled().then((gateways) => {
      return Promise.all(gateways.map((gw, index, array) => {
        if (gw.name) {
  ...
  it('ensures that the local "mygateway" is configured properly (live/recording only)', function () {
    ...
    return powerShell("exit [int] -not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] 'Administrator')").should.be.fulfilled().then(() =>

      // make sure that the SMT gateway service is installed already
      powerShell('get-service ServerManagementToolsGateway')).should.be.fulfilled().then(() =>

        //check if  there is a gateway registered for this service called 'mygateway'
        client.gateway.get(resourceGroupName, "mygateway")).then(gw => {
  ...
  it.skip('gets a new profile installed', function () {
    // download the profile.
    return client.gateway.getProfile(resourceGroupName, 'mygateway').should.be.fulfilled().then(profile => {
  ...
  it("Creates a node for this PC.", function () {
    var nodename = suite.Get("smtnodename", () => require('os').hostname());
    var username = suite.Get("smtusername", () => "gsadmin");
    var password = suite.Get("smtpassword", () => "S0meP@sswerd!");

    // get the gateway
    return client.gateway.get(resourceGroupName, "mygateway").should.be.fulfilled().then(gw => {

      // register the node.
      return client.node.create(resourceGroupName, nodename, { location: location, gatewayId: gw.id, userName: username, password: password, connectionName: nodename }).should.be.fulfilled().then(node => {
  ...
  it('gets the list of nodes', function () {
    return client.node.list(resourceGroupName).should.be.fulfilled().should.be.fulfilled().then(nodes => {
      return Promise.all(nodes.map((node, index, array) => {
  ...
  it("Creates a session and makes a powershell call.", function () {
    var nodename = suite.Get("smtnodename", () => require('os').hostname());
    var sessionName = suite.Get("smtsession", () => rnd("session-"));
    var username = suite.Get("smtusername", () => "gsadmin");
    var password = suite.Get("smtpassword", () => "S0meP@sswerd!");

    log("creating session")
    // get the gateway
    return client.session.create(resourceGroupName, nodename, sessionName, { userName: username, password: password }).should.be.fulfilled().then(session => {
      log("creating pssession")
      // create the powershell runspace 
      return client.powerShell.createSession(resourceGroupName, nodename, sessionName, "00000000-0000-0000-0000-000000000000").should.be.fulfilled().then(psSession => {
        log("invoking command")
        // run a command
        return client.powerShell.invokeCommand(resourceGroupName, nodename, sessionName, psSession.sessionId, { command: "dir c:\\" }).should.be.fulfilled().then(results => {
          log(`results: ${results.results[0].value}`);
        });
      });

    }).then(() => client.session.deleteMethod(resourceGroupName, nodename, sessionName).should.be.fulfilled());
  });
    ...
    return client.node.list(resourceGroupName).should.be.fulfilled().should.be.fulfilled().then(nodes => {
      return Promise.all(nodes.map((node, index, array) => {
        if (node.name == nodename) {
          return client.node.deleteMethod(resourceGroupName, nodename).should.be.fulfilled().then(() => {
  ...
  // disabled - useful in interactive testing 
  it.skip('Removes gateways that should not be there', function () {
    return client.gateway.list(resourceGroupName).then((gateways) => {
      return Promise.all(gateways.map((gw, index, array) => {
        if (gw.name) {
          log(`found gateway ${gw.name}`);
